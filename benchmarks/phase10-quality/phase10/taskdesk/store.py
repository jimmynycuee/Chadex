...
class TaskHistoryStore:
    ...
    def list(self, limit: int = MAX_ITEMS) -> list[dict[str, Any]]:
        ...
        with self._lock:
            ...
            return [self._copy_item(item) for item in items[:bounded_limit]]

    def record(self, result: TaskResult) -> None:
        if result.status not in TERMINAL_TASK_STATUSES:
            return
        item = self._from_result(result)
        with self._lock:
            items = [
                existing
            ...
            self._write_unlocked([item, *items][: self.MAX_ITEMS])
    ...
    def _normalize_item(self, raw: Any) -> dict[str, Any] | None:
        ...
        return {
            ...
            "changed_files": self._normalize_files(raw.get("changed_files", [])),
            "validation_result": validation_result,
        }

    def _from_result(self, result: TaskResult) -> dict[str, Any]:
        return {
            ...
            "changed_files": self._normalize_files(result.changed_files),
            "validation_result": result.validation_result,
        }

    def _normalize_files(self, values: Iterable[Any] | Any) -> list[str]:
    ...
    def _copy_item(self, item: dict[str, Any]) -> dict[str, Any]:
        copied = dict(item)
        copied["changed_files"] = list(item["changed_files"])
        return copied
