...
#[test]
fn from_tool_name_parses_execute_task_package_count() {
    let call = ToolCall::from_tool_name(
        "execute_task",
        json!({
            "project": "demo",
            "goal": "Inspect one project file.",
            "preplan": {
                "estimated_file_count": 4,
                "estimated_subsystem_count": 3,
                "estimated_language_count": 2,
                "validation_domain_count": 2,
                "cross_runtime_boundary": true,
                "stateful_or_schema_change": false,
                "concurrency_or_security_sensitive": false,
                "graphify_informed": true
            },
            "package_count": 3,
            "steps": [{"kind": "read", "items": [{"path": "README.md"}]}],
            "acceptance": {
    ...
    match call {
        ToolCall::ExecuteTask {
            ...
            assert_eq!(package_count, Some(3));
            let preplan = preplan.expect("preplan");
            assert_eq!(preplan.estimated_file_count, 4);
            assert_eq!(preplan.estimated_subsystem_count, 3);
            assert_eq!(preplan.estimated_language_count, 2);
            assert_eq!(preplan.validation_domain_count, 2);
            assert!(preplan.cross_runtime_boundary);
            assert!(preplan.graphify_informed);
...
#[test]
fn from_tool_name_parses_execute_tasks_batch() {
    let call = ToolCall::from_tool_name(
        "execute_tasks",
        json!({
            "project":"demo",
            "tasks":[
                {"goal":"task a","steps":[{"kind":"read","items":[{"path":"a.txt"}]}]},
                {"goal":"task b","steps":[{"kind":"read","items":[{"path":"b.txt"}]}]}
            ]
        }),
    )
    .unwrap();
    match call {
        ToolCall::ExecuteTasks { project, tasks } => {
            assert_eq!(project, "demo");
            assert_eq!(tasks.len(), 2);
            assert_eq!(tasks[0].goal, "task a");
            assert_eq!(tasks[1].goal, "task b");
        }
        other => panic!("expected ExecuteTasks, got {other:?}"),
    }
}

#[test]
fn from_tool_name_parses_task_recovery_actions() {
    let list = ToolCall::from_tool_name(
        "task_recovery",
        json!({"project":"demo","action":"list"}),
    )
    .unwrap();
    assert!(matches!(
        list,
        ToolCall::TaskRecovery {
            project,
            action,
            task_id: None,
        } if project == "demo" && action == "list"
    ));

    let task_id = "chadex_task_0123456789abcdef0123456789abcdef";
    let retry = ToolCall::from_tool_name(
        "task_recovery",
        json!({"project":"demo","action":"retry","task_id":task_id}),
    )
    .unwrap();
    assert!(matches!(
        retry,
        ToolCall::TaskRecovery {
            ...
            action,
            task_id: Some(id),
        } if project == "demo" && action == "retry" && id == task_id
    ));
}
...
#[test]
fn read_files_revision_option_matches_public_contract_and_rejects_unknown_fields() {
    for flag in [true, false] {
        let call = crate::ToolCall::from_tool_name(
            "read_files",
            serde_json::json!({
                "project": "demo", "items": [{"path": "a.txt"}], "include_read_revision": flag
            }),
        )
        .unwrap();
        assert!(
            matches!(call, crate::ToolCall::ReadFiles { include_read_revision: Some(value), .. } if value == flag)
        );
    }
    for invalid in [serde_json::json!("true"), serde_json::json!(1)] {
        assert!(crate::ToolCall::from_tool_name(
            "read_files",
            serde_json::json!({
                "project": "demo", "items": [{"path": "a.txt"}], "include_read_revision": invalid
            })
        )
        .is_err());
    }
    assert!(crate::ToolCall::from_tool_name(
        "read_files",
        serde_json::json!({
            "project": "demo", "items": [{"path": "a.txt"}], "include_read_revison": true
        })
    )
    .is_err());
}
