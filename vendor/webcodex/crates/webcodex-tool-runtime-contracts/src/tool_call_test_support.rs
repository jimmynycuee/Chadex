...
fn sample_tool_args_for_spec(spec: &ToolSpec) -> Value {
    ...
    match spec.name.as_str() {
        ...
        "observe_task" | "cancel_task" => {
            args.insert(
                "task_id".to_string(),
                json!("chadex_task_0123456789abcdef0123456789abcdef"),
            );
        }
        "execute_tasks" => {
            args.insert("project".to_string(), json!(SAMPLE_PROJECT));
            args.insert("tasks".to_string(), json!([
                {"goal":"task a","steps":[{"kind":"read","items":[{"path":"src/lib.rs"}]}]},
                {"goal":"task b","steps":[{"kind":"read","items":[{"path":"src/lib.rs"}]}]}
            ]));
        }
        "task_recovery" => {
            args.insert("project".to_string(), json!(SAMPLE_PROJECT));
...
fn sample_field_value(field: &str) -> Value {
    match field {
        ...
        "goal" => json!("inspect the project and return a bounded result"),
        "steps" => json!([{"kind": "read", "items": [{"path": "src/lib.rs"}]}]),
        "tasks" => json!([
            {"goal":"task a","steps":[{"kind":"read","items":[{"path":"src/lib.rs"}]}]},
            {"goal":"task b","steps":[{"kind":"read","items":[{"path":"src/lib.rs"}]}]}
        ]),
