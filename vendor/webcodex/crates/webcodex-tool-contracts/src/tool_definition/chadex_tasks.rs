...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    permission_risk(
        model_spec(
            ...
            "Execute one bounded deterministic Chadex task. Supply a stable task_id for replay protection; omission creates a new execution. execution_id equals task_id; execution_state is canonical, terminal states are immutable, and succeeded requires a durable result. Never retry unknown outcomes. Cancellation is a request, not proof of termination. Mutations/processes use detached managed worktrees. Package parallelism requires dependency proof and meaningful estimated gain; small tasks stay sequential. Apply-back respects write-set overlap, isolated reconciliation, strict source CAS, permission/path guards, validation and review. Explicit recovery retry creates a new execution linked by previous_execution_id. No permanent branch, push or deploy.",
