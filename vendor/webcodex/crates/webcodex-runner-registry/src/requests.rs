...
use super::{
    now_ts, RunnerFeature, RunnerRegistry, MAX_QUEUED_REQUESTS_PER_RUNNER,
...
use webcodex_core::runner_operation::{
    ...
    RunnerScriptOperation, RunnerSearchTextOperation, RunnerShellOperation,
};
use webcodex_core::runner_protocol::{
    shell_computer_request_payload_max_bytes, PersistentShellRequest, PersistentShellResult,
    validate_text_search_request, RunnerConfigOperationRequest, RunnerRequest,
    RunnerTextSearchRequest, ShellFileOpRequest, ShellJobContext, ShellProcessArgv, ShellRunRequest,
    ShellRunResponse, ShellScriptLanguage, ShellScriptPayload,
    ...
    RUNNER_CAPABILITY_STRUCTURED_SEARCH_TEXT, RUNNER_CAPABILITY_STRUCTURED_SCRIPT_JAVASCRIPT,
...
impl RunnerRegistry {
    ...
    pub async fn enqueue_project_file_read(
        ...
        Ok(batch
            .pop()
            .expect("single project-file read batch returns one request"))
    }

    /// Atomically admit a bounded group of independent project-file reads for
    /// one exact Runner/Project placement. Validation, capacity, ownership,
    /// capability and placement fences are observed once under the same lock;
    /// no request is inserted unless the complete batch can be admitted.
    pub async fn enqueue_project_file_reads(
        &self,
        bodies: Vec<ShellFileOpRequest>,
        expected_project_id: &str,
        expected_project_cwd: &str,
        expected_runner_instance_id: &str,
        requested_by: String,
    ) -> Result<Vec<(String, oneshot::Receiver<ShellRunResponse>)>, String> {
        if bodies.is_empty() || bodies.len() > 8 {
            return Err("project-file read batch must contain 1..=8 requests".to_string());
        }
        let client_id = bodies[0].client_id.clone();
        ...
        for body in bodies {
            ...
            let request_id = next_request_id();
            let (tx, rx) = oneshot::channel();
            let request = encode_file_operation(&request_id, &body, requested_by.clone())?;
            pending.push((request_id, request, tx, rx));
        }
        let mut inner = self.inner.lock().await;
        self.prune_expired_shared_key_runners_locked(&mut inner, now_ts());
        ensure_dispatch_supported_locked(&inner, &client_id).map_err(|error| error.to_string())?;
        let current = inner
            .runners
            .get(&client_id)
        ...
        let queued = inner
            ...
            .unwrap_or(0);
        if queued.saturating_add(pending.len()) > MAX_QUEUED_REQUESTS_PER_RUNNER {
            return Err(format!(
                "runner queue full: {} already has {} pending requests; batch of {} exceeds limit {}",
                client_id,
                queued,
                pending.len(),
                MAX_QUEUED_REQUESTS_PER_RUNNER
            ));
        }
        let mut responses = Vec::with_capacity(pending.len());
        let mut admitted_ids: Vec<String> = Vec::with_capacity(pending.len());
        for (request_id, request, tx, rx) in pending {
            if let Err(error) = enqueue_pending_request_locked(
                self.telemetry.as_ref(),
                &mut inner,
                &client_id,
                request_id.clone(),
                ...
                for admitted_id in &admitted_ids {
                    remove_pending_request_locked(&mut inner, admitted_id);
                }
                return Err(error.to_string());
            }
            admitted_ids.push(request_id.clone());
            let admitted = inner
                .pending_by_id
                .get_mut(&request_id)
                .expect("project-file read was just enqueued");
            admitted.expected_runner_owner = expected_runner_owner.clone();
            admitted.expected_project_id = Some(expected_project_id.to_string());
            admitted.expected_project_cwd = Some(expected_project_cwd.to_string());
            admitted.expected_project_runner_instance_id =
                Some(expected_runner_instance_id.to_string());
            responses.push((request_id, rx));
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn enqueue_search_text(
        &self,
        client_id: String,
        cwd: Option<String>,
        search: RunnerTextSearchRequest,
        timeout_secs: u64,
        wait_timeout_secs: u64,
        requested_by: String,
    ) -> Result<(String, oneshot::Receiver<ShellRunResponse>), String> {
        validate_text_search_request(&search)?;
        if timeout_secs == 0 || timeout_secs > 120 {
            return Err("search_text timeout_secs must be between 1 and 120".to_string());
        }
        if wait_timeout_secs == 0 || wait_timeout_secs > 120 {
            return Err("search_text wait_timeout_secs must be between 1 and 120".to_string());
        }
        let normalized_cwd = cwd.map(|cwd| cwd.trim().to_string());
        let request_id = next_request_id();
        let (tx, rx) = oneshot::channel();
        let request = encode_runner_operation(
            ...
            &client_id,
            requested_by,
            RunnerOperation::SearchText(RunnerSearchTextOperation {
                cwd: normalized_cwd,
                request: search,
                timeout_secs,
            }),
        ...
        if !runner.runner_features.supports(RunnerFeature::StructuredSearchText) {
            return Err(format!(
                "capability_unavailable: runner {client_id} does not support {RUNNER_CAPABILITY_STRUCTURED_SEARCH_TEXT}"
            ));
        }
        enqueue_pending_request_locked(
            self.telemetry.as_ref(),
