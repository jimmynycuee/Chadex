...
use super::{
    ...
    run_shell_with_profiles_and_execution_state, run_ssh_shell_with_execution_state,
    run_text_search_with_profiles_and_execution_state, CommandResult,
...
use crate::runner_protocol::{
    ...
    RunnerJobUpdateRequest, RunnerRequest, RunnerTextSearchPatternMode,
    RunnerTextSearchResultMode, EXTERNAL_SEARCH_REQUEST_PREFIX,
...
fn escape_search_literal_for_external_regex(pattern: &str) -> String {
...
fn external_search_payload(
    request: &crate::runner_protocol::RunnerTextSearchRequest,
    timeout_secs: u64,
) -> String {
    let pattern = match request.pattern_mode {
        RunnerTextSearchPatternMode::Regex => request.pattern.clone(),
        RunnerTextSearchPatternMode::Literal => {
            escape_search_literal_for_external_regex(&request.pattern)
        }
    };
    serde_json::json!({
        "pattern": pattern,
        ...
        "result_mode": match request.result_mode {
            RunnerTextSearchResultMode::Matches => "matches",
            RunnerTextSearchResultMode::FilesWithMatches => "files_with_matches",
            RunnerTextSearchResultMode::Count => "count",
        },
...
pub(crate) fn dispatch_request_with_outcome(
    ...
    let dispatch_result = match invocation.operation {
        ...
        RunnerOperation::SearchText(operation) => {
            let provider_route = RunnerShellOperation {
                cwd: operation.cwd.clone(),
                command: EXTERNAL_SEARCH_REQUEST_PREFIX.to_string(),
                stdin: Some(external_search_payload(&operation.request, operation.timeout_secs)),
                max_bytes: None,
            ...
            match config.external_tools.route_with_shutdown(
                ...
                ExternalRoute::NativeFallback(fallback) => {
                    let result = run_text_search_with_profiles_and_execution_state(
                        config.generation,
                        policy,
                        shell,
                        project_registry_dir,
...
mod typed_search_tests {
    use super::*;
    use crate::runner_protocol::RunnerTextSearchRequest;

    #[test]
    fn typed_search_external_payload_preserves_legacy_regex_provider_contract() {
        ...
        let payload: serde_json::Value =
            serde_json::from_str(&external_search_payload(&request, 29)).unwrap();
        assert_eq!(payload["pattern"], r"RuntimeInfo \{");
        assert!(payload.get("pattern_mode").is_none());
        assert_eq!(payload["path"], "src");
        assert_eq!(payload["limit"], 17);
        assert_eq!(payload["context_before"], 2);
        assert_eq!(payload["context_after"], 3);
        assert_eq!(payload["include_globs"], serde_json::json!(["*.rs"]));
        assert_eq!(
            payload["exclude_globs"],
            serde_json::json!(["generated/**"])
        );
        assert_eq!(payload["result_mode"], "count");
        assert_eq!(payload["timeout_secs"], 29);
    }

    #[test]
    fn typed_search_external_payload_leaves_regex_unchanged() {
        let request = RunnerTextSearchRequest {
            pattern: "needle.+value".to_string(),
            pattern_mode: RunnerTextSearchPatternMode::Regex,
            path: ".".to_string(),
            limit: 20,
            context_before: 0,
            context_after: 0,
        ...
        let payload: serde_json::Value =
            serde_json::from_str(&external_search_payload(&request, 30)).unwrap();
        assert_eq!(payload["pattern"], "needle.+value");
