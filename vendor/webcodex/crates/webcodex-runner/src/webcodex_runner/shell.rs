...
use crate::runner_protocol::{
    ...
    ShellProcessArgv, ShellScriptLanguage, ShellScriptPayload,
};
...
fn force_terminate_child_process_tree(child: &mut ManagedChild) -> Result<(), String> {
    ...
    loop {
        match child.try_tree_exit() {
            ...
            Err(error) => {
                errors.push(format!("failed to wait for command process tree exit: {error}"));
                break;
            }
        }
    }
...
fn execute_search_command(
    ...
    let stderr_handle = std::thread::spawn(move || {
        let _ = stderr_tx.send(read_bounded_pipe_tail(stderr, stderr_limit, "search stderr"));
    });
    ...
    let (mut stderr_text, mut stderr_truncated) =
        stderr.normalize_with_truncation(stderr_limit);
...
pub(crate) fn run_text_search_with_profiles_and_execution_state(
    ...
    let project_root = match cwd_path.canonicalize() {
        ...
        Err(error) => {
            return ShellCommandResult::not_started(CommandResult {
                ...
                stderr: None,
                duration_ms: Some(0),
                error: Some(format!("failed to resolve native search project root: {error}")),
            })
        }
    };
    ...
    match configured_search_program(
        shell,
        profile.as_deref(),
        "rg",
        &rg_args,
