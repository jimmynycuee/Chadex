...
impl TunnelConfig {
    ...
    /// Chadex uses this only as an empty, inert placeholder while WebCodex is
    /// still the temporary local MCP/file backend. Chadex never injects its
    /// Tunnel ID or API key into this configuration.
