...
const STARTUP_FAST_POLL_INTERVAL: Duration = Duration::from_millis(25);
const STARTUP_MEDIUM_POLL_INTERVAL: Duration = Duration::from_millis(100);
const STARTUP_FAST_POLL_WINDOW: Duration = Duration::from_secs(1);
const STARTUP_MEDIUM_POLL_WINDOW: Duration = Duration::from_secs(3);
...
impl AppState {
    ...
    /// Chadex never provides Tunnel ID or API key to this AppState.
    pub fn new_for_chadex_backend(data_dir: PathBuf, resource_dir: PathBuf) -> DesktopResult<Self> {
        let core =
            DesktopCore::new_with_tunnel_config(data_dir, resource_dir, TunnelConfig::in_memory())?;
        Self::from_core(core)
    }

    fn from_core(core: DesktopCore) -> DesktopResult<Self> {
...
fn startup_readiness_poll_interval(elapsed: Duration) -> Duration {
