...
async fn read_file_dispatch_complete_success_is_sparse_after_session_recording() {
    ...
    assert!(item["output"].get("read_revision").is_none());
...
#[tokio::test]
async fn read_file_dispatch_can_opt_in_to_complete_read_revision() {
    let root = tempfile::tempdir().unwrap();
    let runtime = ToolRuntime::new_for_tests();
    let client_id = "read-complete-revision-opt-in";
    let project = register_runner_project_at_path(&runtime, client_id, "demo", root.path()).await;
    let auth = auth_context(None, true);
            ...
            runtime
                .dispatch_with_auth(
                    ToolCall::ReadFiles {
                        ...
                        session_id: None,
                        with_line_numbers: None,
                        include_read_revision: Some(true),
                        max_result_bytes: None,
                    },
                    Some(&auth),
                )
    ...
    let request = next_read_request(&runtime, client_id).await;
    complete_read(&runtime, client_id, &request, "one\ntwo").await;
    let result = task.await.unwrap();
