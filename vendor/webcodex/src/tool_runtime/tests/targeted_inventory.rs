...
#[tokio::test]
async fn exact_project_resolution_cache_is_partitioned_by_authority() {
    let runtime = test_runtime();
    let auth_a = shared_key_auth_context("targeted-a");
    let auth_b = shared_key_auth_context("targeted-b");
    register_target_agent_for_auth(&runtime, "client-a", "visible-a", &auth_a).await;
    ...
    let visible = runtime
        .resolve_project_input_for_auth("agent:client-a:visible-a", Some(&auth_a))
        .await
        .expect("authorized exact project should resolve");
    assert_eq!(visible.resolved_id, "agent:client-a:visible-a");

    let hidden = runtime
        .resolve_project_input_for_auth("agent:client-a:visible-a", Some(&auth_b))
        .await
        .expect_err("a cached exact project must not cross authority partitions");
    assert_eq!(
        hidden.kind,
        super::super::project_resolution::ProjectResolverErrorKind::UnknownProject
    );
