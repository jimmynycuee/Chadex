...
impl ToolRuntime {
    ...
    async fn call_tool_with_context_inner(
        ...
        let mut session_event = if context.session_id.is_some() {
            ...
            // Ordinary no-session calls cannot produce a Workflow Session event.
            // Avoid building the redacted/audit argument projection on this hot
            // path; concrete parsing, authorization, permissions and execution
        ...
        let outer_recording = if session_event.is_some() {
            let session_log_result =
                session_log_result_for_tool(&request.tool_name, &result.output);
            ...
            // No outer recording Session means this projection would be built
            // only to be discarded by the SessionStore. Keep the ordinary
            // no-session read path allocation-free with respect to Session
            // audit payloads.
            None
        };
