...
/// file. `read_revision`, when requested, remains the model-facing snapshot
/// identity; inspection-only complete reads may omit it. `total_lines` remains
/// content-shape evidence. Partial reads and every real continuation keep the canonical full
...
pub(crate) fn sparsify_complete_file_read_output(
    ...
    // The digest has already proved the canonical complete-file shape. It stays
    // internal whether or not the caller opted into a read_revision handle.
    ...
    if let Some(items) = output.get_mut("items").and_then(Value::as_array_mut) {
        for item in items {
            if let Some(read) = item.get_mut("output").and_then(Value::as_object_mut) {
                ...
                // The digest is transport/internal snapshot evidence only.
                // Lazy revision policy must never make it model-facing when a
                // complete inspection intentionally omits read_revision.
                read.remove("sha256");
            }
        }
    }
        ...
        match call {
            ...
            ToolCall::ExecuteTask {
                ...
                let project = match project_resolution {
                    Some(Ok(project)) => project,
                    Some(Err(error)) => return error.into_tool_result(),
                    None => return ToolResult::err("execute_task requires a resolved Project"),
                };
                Box::pin(self.execute_chadex_task(
                    project,
                    task_id,
                    goal,
                    preplan,
            ...
            ToolCall::ExecuteTasks { project: _, tasks } => {
                let project = match project_resolution {
                    Some(Ok(project)) => project,
                    Some(Err(error)) => return error.into_tool_result(),
                    None => return ToolResult::err("execute_tasks requires a resolved Project"),
                };
                Box::pin(self.execute_chadex_tasks(project, tasks, auth, transport)).await
            }
            ToolCall::ObserveTask {
                project: _,
                task_id,
            } => {
                let project = match project_resolution {
                    Some(Ok(project)) => project,
                    Some(Err(error)) => return error.into_tool_result(),
                    None => return ToolResult::err("observe_task requires a resolved Project"),
                };
                self.observe_chadex_task(&project.resolved_id, &task_id)
            }
            ToolCall::CancelTask {
                project: _,
                task_id,
                ...
                let project = match project_resolution {
                    Some(Ok(project)) => project,
                    Some(Err(error)) => return error.into_tool_result(),
                    None => return ToolResult::err("cancel_task requires a resolved Project"),
                };
                self.cancel_chadex_task(&project.resolved_id, &task_id)
            }
            ToolCall::TaskRecovery {
                ...
                let project = match project_resolution {
                    Some(Ok(project)) => project,
                    Some(Err(error)) => return error.into_tool_result(),
                    None => return ToolResult::err("task_recovery requires a resolved Project"),
                };
                Box::pin(self.chadex_task_recovery(project, task_id, action, auth, transport)).await
            }
