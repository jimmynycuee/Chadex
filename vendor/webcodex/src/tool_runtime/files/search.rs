...
impl ToolRuntime {
    pub(crate) async fn search_one_resolved_project_text(
        ...
        let typed_search = crate::runner_protocol::RunnerTextSearchRequest {
            pattern: options.pattern.clone(),
            pattern_mode: match options.pattern_mode {
                SearchPatternMode::Regex => {
                    crate::runner_protocol::RunnerTextSearchPatternMode::Regex
                }
                SearchPatternMode::Literal => {
                    crate::runner_protocol::RunnerTextSearchPatternMode::Literal
                }
            },
            ...
            result_mode: match options.result_mode {
                SearchResultMode::Matches => {
                    crate::runner_protocol::RunnerTextSearchResultMode::Matches
                }
                SearchResultMode::FilesWithMatches => {
                    crate::runner_protocol::RunnerTextSearchResultMode::FilesWithMatches
                }
                SearchResultMode::Count => {
        ...
        let typed = self
            ...
            .await;
        let (req_id, rx) = match typed {
            Ok(r) => r,
            Err(error) if error.contains("capability_unavailable") => {
                // Rolling-upgrade compatibility: only old Runners that do not
                // advertise structured_search_text retain the generated POSIX
                // implementation and its established external-provider payload.
