...
use crate::chadex_core::runtime_compat::models::{DesktopOperationKind, DesktopOperationPhase, DesktopOperationSnapshot};
...
impl OperationController {
    ...
    pub(crate) fn admit(
        ...
        if let Some(active) = slot.as_ref() {
            ...
            .with_details(serde_json::json!({
                "operation_id": active.snapshot.id,
                "operation_kind": active.snapshot.kind.as_str(),
            })));
        }
    ...
    pub(crate) fn cancel(&self, observed_id: &str) -> DesktopResult<DesktopOperationSnapshot> {
        ...
        if active.snapshot.id != observed_id {
            return Err(operation_not_current(
                observed_id,
                Some(active.snapshot.id.as_str()),
            ));
        }
    ...
    pub(crate) fn finish<T>(&self, operation_id: &str, result: &DesktopResult<T>) {
        ...
        if let Err(error) = result {
            let (event_kind, level, message) = if error.code == "desktop_operation_cancelled" {
                (
            ...
            self.activity.push(event_kind, "desktop", level, message);
        }
...
mod tests {
    ...
    #[test]
    fn stale_cancel_cannot_target_a_new_operation() {
    ...
    #[test]
    fn cancelling_keeps_the_mutation_slot_until_finish() {
        let controller = OperationController::new(ActivityLog::default());
        let first = controller
            .admit(DesktopOperationKind::LocalSetup, true)
            .unwrap();
