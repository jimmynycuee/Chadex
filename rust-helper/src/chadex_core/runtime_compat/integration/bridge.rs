use super::command::{run_json, run_json_until, run_project_activation_json, RuntimeToolchain};
use super::contracts::{
...
use crate::chadex_core::runtime_compat::models::ProjectSelection;
use crate::chadex_core::runtime_compat::operation::CancellationContext;
...
pub struct RuntimeIntegrationBridge {
    binaries: Option<RuntimeToolchain>,
    bundled_runtime_dir: Option<PathBuf>,
    task_state_dir: PathBuf,
}

impl RuntimeIntegrationBridge {
    pub fn new(bundled_runtime_dir: Option<PathBuf>, task_state_dir: PathBuf) -> Self {
        Self {
            binaries: None,
            bundled_runtime_dir,
            task_state_dir,
    ...
    ) -> DesktopResult<&RuntimeToolchain> {
        if self.binaries.is_none() {
            self.binaries = Some(
                RuntimeToolchain::resolve(self.bundled_runtime_dir.as_deref(), cancellation)
                    .await?,
            );
        }
    ...
    pub async fn ensure_binaries_until(
        ...
        if self.binaries.is_none() {
            self.binaries = Some(
                RuntimeToolchain::resolve_until(
    ...
    pub fn binaries(&self) -> DesktopResult<&RuntimeToolchain> {
    ...
    pub async fn init_local_server(
        ...
        let webcodex = self.ensure_binaries(cancellation).await?.cli.clone();
        cancellation.check()?;
    ...
    async fn server_status_with_deadline(
        ...
        let webcodex = match deadline {
            Some(deadline) => self
                ...
                .cli
                .clone(),
            None => self.ensure_binaries(cancellation).await?.cli.clone(),
        };
    ...
    pub fn local_server_command(&self, env_file: &Path) -> DesktopResult<Command> {
        ...
        binaries.verify_server_generation()?;
    ...
    pub fn local_runner_command(&self, config: &Path) -> DesktopResult<Command> {
        let binaries = self.binaries()?;
        binaries.verify_runner_generation()?;
    ...
    pub fn quick_share_command(
        ...
        let mut command = Command::new(&binaries.cli);
        command
            .arg("share")
            .arg("--root")
            .arg(project)
    ...
    async fn observe_runner_connection_with_deadline(
        ...
        let runtime = output
            .runtime
            .unwrap_or(super::contracts::RunnerRuntimeOutput {
                checked: false,
                reachable: None,
                client_online: None,
            });
...
pub async fn inspect_project_path(path: &str) -> DesktopResult<ProjectSelection> {
    ...
    chadex_runtime_runner_config::paths::validate_project_path_ingress(&requested).map_err(|_| {
        DesktopError::new(
...
fn ops_project_is_ready(
    candidate: &super::contracts::OpsProject,
    identity: &ProjectRuntimeIdentity,
) -> bool {
...
mod tests {
    ...
    fn quick_share_only_inherits_control_plane_credentials_for_openai_provider() {
        let binaries = RuntimeToolchain::test_fixture("0.3.9");
    ...
    fn local_runner_bypasses_loopback_without_overriding_proxy_servers() {
        let binaries = RuntimeToolchain::test_fixture("0.4.1");
        let adapter = RuntimeIntegrationBridge {
            binaries: Some(binaries),
            bundled_runtime_dir: None,
            task_state_dir: PathBuf::from("task-state"),
    ...
    fn ops_project_readiness_uses_runtime_project_identity() {
        ...
        let ready = super::super::contracts::OpsProject {
        ...
        let short_id = super::super::contracts::OpsProject {
            id: "repo".to_string(),
            ..ready
        };
    ...
    fn windows_network_and_local_projects_use_exact_roots() {
        ...
        assert!(chadex_runtime_runner_config::paths::paths_equal(
