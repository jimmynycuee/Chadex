...
use crate::chadex_core::runtime_compat::models::BinaryInfo;
...
impl ToolchainSource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::Environment => "CHADEX_RUNTIME_BIN_DIR",
            Self::LegacyEnvironment => "legacy WEBCODEX_DESKTOP_BIN_DIR",
            Self::SourceDogfoodTarget => "Chadex source dogfood",
        }
    }
}
...
pub struct RuntimeToolchain {
    ...
    server_generation: Option<RuntimeBinaryGeneration>,
    runner_generation: Option<RuntimeBinaryGeneration>,
}
...
impl RuntimeToolchain {
    ...
    pub async fn resolve_until(
        ...
        let (directory, source) =
            if let Some(directory) = bundled_runtime_dir.filter(|path| path.is_dir()) {
                (directory.to_path_buf(), ToolchainSource::Bundled)
            } else if !cfg!(debug_assertions) {
                let expected = bundled_runtime_dir
                    .map(|path| path.display().to_string())
                    .unwrap_or_else(|| "<Chadex resource directory>/chadex-runtime".to_string());
                return Err(DesktopError::new(
                    "bundled_runtime_missing",
                    format!("The installed Chadex runtime is missing: {expected}"),
                    "Reinstall Chadex from the matching release build.",
                ));
            } else if let Some(value) = std::env::var_os("CHADEX_RUNTIME_BIN_DIR") {
                let directory = PathBuf::from(value);
                if directory.as_os_str().is_empty() {
                    return Err(DesktopError::new(
                        "binary_directory_invalid",
                        "CHADEX_RUNTIME_BIN_DIR is empty",
                        "Set it to the directory containing the Chadex runtime binaries.",
                    ));
                }
                (directory, ToolchainSource::Environment)
                ...
                if directory.as_os_str().is_empty() {
                    return Err(DesktopError::new(
                        ...
                        "Set CHADEX_RUNTIME_BIN_DIR to the Chadex runtime directory.",
                    ));
                }
                (directory, ToolchainSource::LegacyEnvironment)
            } else {
                let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
                let repo = manifest.parent().ok_or_else(|| {
                    DesktopError::new(
                        "binary_directory_invalid",
                        "Could not derive the Chadex source root",
                        "Set CHADEX_RUNTIME_BIN_DIR explicitly.",
                    )
                })?;
                (
                    repo.join("runtime-engine").join("target").join("dogfood"),
                    ToolchainSource::SourceDogfoodTarget,
                )
            };
        let missing_directory_action = match source {
            ToolchainSource::Bundled => "Reinstall Chadex from the matching release build.",
            ToolchainSource::Environment | ToolchainSource::SourceDogfoodTarget => {
                "Build the Chadex runtime entrypoints or set CHADEX_RUNTIME_BIN_DIR."
            }
            ToolchainSource::LegacyEnvironment => {
                "Build the legacy compatibility binaries or set CHADEX_RUNTIME_BIN_DIR."
            }
        };
        let directory = directory.canonicalize().map_err(|_| {
            DesktopError::new(
                ...
                format!(
                    "Runtime binary directory does not exist: {}",
                    directory.display()
                ),
                missing_directory_action,
            )
        })?;
        let (cli_name, server_name, runner_name) = match source {
            ToolchainSource::LegacyEnvironment => ("webcodex", "webcodex-server", "webcodex-runner"),
            _ => (
                "chadex-runtime-cli",
        ...
        let cli = directory.join(executable_name(cli_name));
        let server = directory.join(executable_name(server_name));
        let runner = directory.join(executable_name(runner_name));
        for path in [&cli, &server, &runner] {
            if !path.is_file() {
                return Err(DesktopError::new(
                    "binary_missing",
                    format!("Required runtime binary is missing: {}", path.display()),
                    match source {
                        ToolchainSource::Bundled => {
                            "Reinstall Chadex from the matching release build."
                        }
                        ToolchainSource::Environment | ToolchainSource::SourceDogfoodTarget => {
                            "Build all Chadex runtime entrypoints from the current source baseline."
                        }
                        ToolchainSource::LegacyEnvironment => {
                            "Build the legacy compatibility binaries from one source baseline."
                        }
                    },
                ));
            }
        ...
        let server_generation = runtime_binary_generation(&server)?;
        let runner_generation = runtime_binary_generation(&runner)?;
        let cli_version = binary_version(&cli, cancellation, deadline).await?;
            ...
            || cli_version.git_commit != runner_version.git_commit
        {
            return Err(DesktopError::new(
                ...
                "Rebuild all runtime entrypoints from one compatible source baseline.",
            ));
        }
        if cli_version.git_commit == "unknown" {
            return Err(DesktopError::new(
                "binary_version_unverifiable",
                "Runtime binaries do not carry a source revision",
                "Rebuild the Chadex runtime entrypoints with pinned compatibility metadata.",
            ));
        }
        verify_runtime_binary_generation(&server, &server_generation, "Server")?;
        verify_runtime_binary_generation(&runner, &runner_generation, "Runner")?;
        Ok(Self {
            directory,
            cli,
            server,
    ...
    pub fn verify_server_generation(&self) -> DesktopResult<()> {
        match self.server_generation.as_ref() {
            Some(expected) => verify_runtime_binary_generation(&self.server, expected, "Server"),
            None => Ok(()),
        }
    }

    pub fn verify_runner_generation(&self) -> DesktopResult<()> {
        match self.runner_generation.as_ref() {
            Some(expected) => verify_runtime_binary_generation(&self.runner, expected, "Runner"),
            None => Ok(()),
        }
    }

    #[cfg(test)]
    pub(crate) fn test_fixture(version: &str) -> Self {
        Self {
            directory: PathBuf::from("bin"),
            cli: PathBuf::from("chadex-runtime-cli"),
...
struct RuntimeBinaryGeneration {
    length: u64,
...
fn runtime_binary_generation(path: &Path) -> DesktopResult<RuntimeBinaryGeneration> {
    let metadata = std::fs::metadata(path).map_err(|error| {
        DesktopError::new(
            "runtime_binary_unreadable",
            format!("Could not inspect runtime binary: {}", path.display()),
            "Reinstall or rebuild Chadex, then retry.",
        )
    ...
    if !metadata.is_file() {
        return Err(DesktopError::new(
            "runtime_binary_unreadable",
            format!("Runtime binary is not a file: {}", path.display()),
            "Reinstall or rebuild Chadex, then retry.",
        ));
    }
    Ok(RuntimeBinaryGeneration {
        length: metadata.len(),
...
fn verify_runtime_binary_generation(
    path: &Path,
    expected: &RuntimeBinaryGeneration,
    component: &str,
) -> DesktopResult<()> {
    if runtime_binary_generation(path)? == *expected {
        return Ok(());
    }
    Err(DesktopError::new(
        "runtime_bundle_changed",
        format!("The bundled Chadex {component} changed while this app instance was running"),
        "Quit and reopen Chadex before continuing. Do not replace a running Chadex.app bundle.",
    ))
}
...
mod tests {
    use super::*;
    use crate::chadex_core::runtime_compat::operation::{CancellationContext, CancellationSignal};
    ...
    fn bundled_source_label_is_stable() {
        assert_eq!(ToolchainSource::Bundled.label(), "Bundled");
    }

    #[test]
    fn runtime_binary_generation_detects_replacement() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("runner");
        std::fs::write(&path, b"generation-one").unwrap();
        let expected = runtime_binary_generation(&path).unwrap();
        verify_runtime_binary_generation(&path, &expected, "Runner").unwrap();

        std::fs::remove_file(&path).unwrap();
        std::fs::write(&path, b"generation-two-is-different").unwrap();
        let error = verify_runtime_binary_generation(&path, &expected, "Runner").unwrap_err();
        assert_eq!(error.code, "runtime_bundle_changed");
    }
    ...
    async fn desktop_real_process_windows_blocked_stdin_reclaims_the_owned_process_tree() {
        let marker = std::env::temp_dir().join(format!(
            "chadex-command-blocked-stdin-{}-{}.txt",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
