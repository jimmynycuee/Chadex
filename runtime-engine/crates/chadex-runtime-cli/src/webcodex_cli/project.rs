...
mod tests {
    ...
    #[test]
    fn activation_allowed_root_update_preserves_managed_worktree_root() {
        let tmp = canonical_test_tempdir();
        let config_path = tmp.path().join("runner.toml");
        let source = tmp.path().join("source");
        let next = tmp.path().join("next");
        let managed = tmp.path().join("runtime-state").join("managed-worktrees");
        std::fs::create_dir_all(&source).unwrap();
        std::fs::create_dir_all(&next).unwrap();
        activation_config(
            &config_path,
            "http://127.0.0.1:1",
            &[source.canonicalize().unwrap(), managed.clone()],
            false,
        );
        let original = std::fs::read_to_string(&config_path).unwrap();
        ...
        let updated = persist_registration_allowed_roots_if_unchanged(
            &config_path,
            &original,
            &[
        ...
        assert!(roots.contains(&next.canonicalize().unwrap().to_str().unwrap()));
    }
