...
pub(crate) fn stage_connection(
    ...
    let managed_worktree_root = published_project_registry_dir
        .parent()
        .ok_or_else(|| "published project registry has no connection directory".to_string())?
    ...
    crate::runner_config::run_runner_init(crate::runner_config::RunnerInitOptions {
        ...
        managed_worktree_root: Some(managed_worktree_root),
...
mod tests {
    ...
    fn a_fresh_login_publishes_through_staging_and_leaves_nothing_behind() {
        ...
        let managed_worktree_root = paths.dir.join("managed-worktrees");
        assert_eq!(
            parsed["policy"]["managed_worktree_root"].as_str(),
            managed_worktree_root.to_str()
        );
        assert!(parsed["policy"]["allowed_roots"]
            .as_array()
            .unwrap()
            .iter()
            .any(|root| root.as_str() == managed_worktree_root.to_str()));
    ...
    async fn login_project_registers_existing_workspace_into_published_registry() {
        ...
        let managed_worktree_root = paths.dir.join("managed-worktrees");
        assert_eq!(local_allowed_roots.len(), 3);
        for expected in [
            allowed_root.clone(),
            project.canonicalize().unwrap(),
            managed_worktree_root.clone(),
        ] {
            assert!(local_allowed_roots.iter().any(|value| {
        ...
        assert!(webcodex_runner_config::paths::paths_equal(
            Path::new(parsed["policy"]["managed_worktree_root"].as_str().unwrap()),
