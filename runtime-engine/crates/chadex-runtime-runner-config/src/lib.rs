...
pub fn validate_runner_init_options(opts: &RunnerInitOptions) -> Result<(), String> {
    ...
    {
        return Err("managed_worktree_root must be a non-empty absolute path".to_string());
    }
    Ok(())
}
...
pub fn generated_runner_config_toml(opts: &RunnerInitOptions) -> Result<String, String> {
    let mut effective_roots =
        effective_allowed_roots(&opts.allowed_roots, opts.allow_cwd_anywhere)?;
    if let Some(managed_root) = opts.managed_worktree_root.as_ref() {
        if !effective_roots.iter().any(|root| root == managed_root) {
            effective_roots.push(managed_root.clone());
        }
    }
    let cfg = GeneratedRunnerConfig {
        ...
        capabilities: RunnerCapabilities {
            ...
            // Native project text search is implemented by the running binary;
            // generated static config must not make an older Runner claim it.
        ...
        policy: GeneratedRunnerPolicy {
            ...
            managed_worktree_root: opts.managed_worktree_root.clone(),
...
mod tests {
    ...
    #[test]
    fn generated_config_adds_managed_worktree_root_without_broadening_project_parent() {
        let mut opts = init_opts(PathBuf::from("-"));
        let managed = std::env::temp_dir().join("chadex-managed-worktrees-config-test");
        opts.managed_worktree_root = Some(managed.clone());
        let content = generated_runner_config_toml(&opts).unwrap();
        let parsed: toml::Value = toml::from_str(&content).unwrap();
        let roots = parsed["policy"]["allowed_roots"]
            .as_array()
            .unwrap()
            .iter()
            .map(|value| value.as_str().unwrap())
            .collect::<Vec<_>>();
        assert!(roots.contains(&"/srv/projects"));
        assert!(roots.contains(&managed.to_str().unwrap()));
        assert_eq!(
            parsed["policy"]["managed_worktree_root"].as_str(),
            managed.to_str()
        );
    }
