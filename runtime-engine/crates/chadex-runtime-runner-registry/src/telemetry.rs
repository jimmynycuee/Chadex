...
pub trait RunnerRegistryTelemetry: Debug + Send + Sync {
    ...
    /// Called only after the registry commits the active Runner lease/poll
    /// dispatch. Default keeps external telemetry implementations compatible.
    fn request_dispatched(&self, _request_id: &str) {}
