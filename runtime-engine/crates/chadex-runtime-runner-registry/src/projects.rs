...
impl RunnerRegistry {
    ...
    /// Authorize one Runner and evaluate an OR-set of capability names under
    /// the same authoritative registry observation. An empty capability slice
    /// performs only the Runner access check.
    pub async fn runner_supports_any_for_auth(
        &self,
        client_id: &str,
        capabilities: &[&str],
        auth: Option<&crate::RunnerAccess>,
        ...
        Ok(capabilities.is_empty()
            || capabilities
                .iter()
                .any(|capability| runner.runner_features.supports_wire_name(capability)))
    }
    ...
    pub async fn upsert_runner_project_for_instance(
        ...
        self.bump_project_routing_epoch();
        Ok(())
    }
