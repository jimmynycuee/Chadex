...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        context_reobservable(model_spec(
            ...
            "Canonical bootstrap for ordinary coding/review. Reuse exact IDs; skip routine runtime_status/list_projects calls. Use project or client_id+path. Omit session_id for a fresh Workflow Session; a fresh Workflow Session does not imply a fresh model context. Use exact resume only for an active accessible Session and never guesses prior Session. Defaults return project instructions, workflow guidance, and Skills/Plugin selection metadata. If the current model context retains instructions/guidance, set the matching include_* false; use defaults for a fresh or uncertain model context. Session/window/transport identity never proves retention; Runtime still re-observes instruction files. Skill bodies require skill_read_file; Plugin calls require plugin_tool describe. Checkout does not require Git; mode=worktree uses an exact Git base for an isolated worktree without bypassing Project authority.",
