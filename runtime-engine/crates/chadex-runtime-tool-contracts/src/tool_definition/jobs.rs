...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Continue known Jobs by job_id; do not call list_jobs first. Return observation_token unchanged as after_observation_token. Observe 1-8 Jobs with bounded deltas/errors. No token gives an immediate baseline; no wait_secs gives an immediate observation. With tokens, bounded wait_secs: 20 seconds is canonical; values above 30 are clamped to 30, and wake_on=change remains the default. Use wake_on=terminal only when useful progress is blocked on terminal outcome; if independent work remains, retain the exact continuation and observe later—never sleep or shell-poll for visibility. For external async work, prefer native connector/status when available. terminal wakes on any terminal result; all_terminal waits for a predetermined set. Updates never extend the deadline; item errors return immediately. Timeout may include changed=true deltas. Never launches, retries, stops, or subscribes.",
            observe_jobs_input_schema,
        ).with_gpt_action_description("Continue known Jobs by job_id/token. Use wait_secs=20,wake_on=terminal only when blocked; cap 30s. Otherwise keep exact continuation and continue independent work. Never sleep or shell-poll for visibility; prefer native external status connectors. Tokens never authorize retry/execution."),
        80,
    ),
];
