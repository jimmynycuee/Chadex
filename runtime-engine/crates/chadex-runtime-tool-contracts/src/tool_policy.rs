...
// Phase 16B keeps the initial model-facing surface intentionally small. ToolDefinitions
// may retain a historical direct rank as ordering metadata, but only this bounded core is
// projected directly at startup; every other model-visible tool remains discoverable and
// executable through tool_manifest -> call_runtime_tool.
const ADAPTIVE_RUNTIME_INITIAL_DIRECT_TOOLS: &[&str] = &[
    // High-frequency coding loop.
    "work_on_project",
    "runtime_status",
    "tool_manifest",
    ...
    "show_changes",
    "finish_coding_task",
    // Host/protocol integration primitives that cannot be faithfully reconstructed
    // from the generic gateway alone (host fileParams, plugin-only OAuth, durable
    // Session recovery).
    ...
    // their direct descriptors carry UI resource metadata or continuation-card identity.
    "present_goal_plan",
    "present_work_result",
    "present_changes",
    "present_agent_continuation",
    "wait_for_agent_events",
    // Preserve experimental Code Mode behavior when that feature is compiled in.
    "code_mode_exec",
    "code_mode_exec_effectful",
    "code_mode_exec_mutating",
];

fn adaptive_runtime_initial_direct_tool(name: &str) -> bool {
    ADAPTIVE_RUNTIME_INITIAL_DIRECT_TOOLS.contains(&name)
}

pub fn runtime_tool_adaptive_direct_rank(name: &str) -> Option<u16> {
    adaptive_runtime_initial_direct_tool(name)
        .then(|| lookup_tool_definition(name))
        .flatten()
...
pub fn adaptive_runtime_direct_tool_definitions() -> Vec<&'static ToolDefinition> {
    let mut definitions = model_visible_tool_definitions()
        .filter(|definition| runtime_tool_adaptive_direct_rank(definition.name).is_some())
        .collect::<Vec<_>>();
    definitions.sort_by_key(|definition| {
        (
            runtime_tool_adaptive_direct_rank(definition.name)
