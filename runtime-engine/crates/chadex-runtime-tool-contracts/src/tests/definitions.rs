...
fn adaptive_runtime_direct_declarations_are_visible_ranked_and_unique() {
    ...
    for pair in derived.windows(2) {
        assert!(
            runtime_tool_adaptive_direct_rank(pair[0].name)
                < runtime_tool_adaptive_direct_rank(pair[1].name)
        );
    }
    assert!(
        derived.len() < seen_ranks.len(),
        "Phase 16B must keep the effective startup surface smaller than the ranked candidate set"
    );
    ...
    assert_eq!(
        derived
            .iter()
            .filter(|definition| expected_core.contains(&definition.name))
            .count(),
        expected_core.len()
    );
    #[cfg(not(feature = "experimental-code-mode"))]
    assert_eq!(derived.len(), expected_core.len());
    #[cfg(feature = "experimental-code-mode")]
    {
        for name in [
            "code_mode_exec",
    ...
    for name in [
        ...
        let definition = lookup_tool_definition(name).expect("ranked direct candidate definition");
        assert!(
            definition.adaptive_runtime_direct_rank().is_some(),
            "{name} should retain its historical rank metadata"
        );
        assert!(
            !is_adaptive_runtime_direct_tool(name),
            "{name} should route through discovery/gateway in the Phase 16B startup surface"
        );
    }
    ...
    for (name, expected_rank, expected_authority) in [
        ...
        let definition =
            lookup_tool_definition(name).unwrap_or_else(|| panic!("{name} definition"));
        assert_eq!(
            definition.adaptive_runtime_direct_rank(),
            Some(expected_rank)
        );
        assert!(
            !is_adaptive_runtime_direct_tool(name),
            "{name} should retain metadata without inflating the Phase 16B startup surface"
        );
