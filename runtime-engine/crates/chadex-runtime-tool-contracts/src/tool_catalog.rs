...
pub const TOOL_DISCOVERY_GROUP_TASK_EXECUTION: &str = "task_execution";
...
pub const TOOL_DISCOVERY_GROUPS: &[ToolDiscoveryGroup] = &[
    ...
    ToolDiscoveryGroup {
        name: TOOL_DISCOVERY_GROUP_TASK_EXECUTION,
        tools: &[
            "execute_task",
            "execute_tasks",
            "observe_task",
...
pub const TOOL_RECOMMENDED_FLOWS: &[ToolRecommendedFlow] = &[
    ...
    ToolRecommendedFlow {
        ...
        summary: "Known plan: after inspection, execute_task can combine edit, validate and review in one call. Keep model-dependent decisions outside the plan. Use direct tools when isolation overhead exceeds saved round trips; never replay an unknown mutation.",
        manifest_purpose:
            "Once edits and validation commands are already decided, use tool_manifest with tool_name=execute_task once if its contract is unknown, then call_runtime_tool with tool=execute_task to run the ordered plan. The goal is not an LLM prompt. Retain nested permission/path guards, detached worktree isolation, validation, review and durable recovery. Source read_revision values are scoped to their Project and must not be reused as worktree revisions. Task-local edit guards read the execution snapshot; if source-read freshness is required, keep the direct guarded edit path. Small tasks stay sequential; compare workspace lifecycle cost with saved external calls. Consume returned validation, review, failure and workspace state before adding follow-up reads or status calls. Never retry unknown outcomes or poll a completed execution.",
        tools: &["execute_task", "observe_task", "task_recovery"],
    },
    ToolRecommendedFlow {
        name: "file_transfer",
