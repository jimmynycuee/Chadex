...
pub struct LoadedInstructionCandidate {
    ...
    /// Whether model-facing projections may offer a project-relative read-more hint.
    pub read_more_supported: bool,
}
...
fn build_instruction_file(
    ...
    let read_more = if truncated && read_more_supported {
        Some(ReadMoreHint {
            path: path.to_string(),
