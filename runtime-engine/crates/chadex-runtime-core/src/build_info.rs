...
pub fn current() -> BuildInfo {
    BuildInfo {
        ...
        git_commit: option_env!("CHADEX_RUNTIME_BUILD_GIT_COMMIT").and_then(non_empty),
        git_dirty: option_env!("CHADEX_RUNTIME_BUILD_GIT_DIRTY").and_then(parse_bool),
        built_at: option_env!("CHADEX_RUNTIME_BUILD_BUILT_AT").and_then(non_empty),
    }
}
...
mod tests {
    ...
    fn build_info_version_output_includes_build_commit_or_unknown() {
        let output = version_output("chadex-runtime-test");
        assert!(output.starts_with(&format!(
            "chadex-runtime-test {} (commit ",
            env!("CARGO_PKG_VERSION")
        )));
        assert!(output.trim_end().ends_with(')'));
        assert_ne!(
            output,
            format!("chadex-runtime-test {}\n", env!("CARGO_PKG_VERSION"))
        );
    }
