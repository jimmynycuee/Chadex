...
/// Runner-native bounded project text search. Missing on older Runners is
/// false; Servers retain the established generated-POSIX fallback.
pub const RUNNER_CAPABILITY_STRUCTURED_SEARCH_TEXT: &str = "structured_search_text";
...
pub const RUNNER_CAPABILITY_NAMES: &[&str] = &[
    ...
    RUNNER_CAPABILITY_STRUCTURED_GO_TEST_PACKAGES,
    RUNNER_CAPABILITY_STRUCTURED_PROCESS_ARGV,
    RUNNER_CAPABILITY_STRUCTURED_SEARCH_TEXT,
    RUNNER_CAPABILITY_STRUCTURED_SCRIPT_PAYLOAD,
...
pub struct RunnerCapabilities {
    ...
    /// Runner-native bounded project text search. Runtime-advertised only;
    /// older Runners omit it and Servers retain the POSIX fallback.
    #[serde(default, skip_serializing_if = "is_false")]
    pub structured_search_text: bool,
...
/// Shared classification for the subset of structured process commands whose
/// Runner timeout may be extended while measurable progress continues. Keeping
/// this contract in runtime-core prevents the server from giving up on a
/// process before the Runner's own bounded adaptive deadline.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum StructuredProcessExecutionClass {
    Generic,
    RustBuild,
    SwiftBuild,
    Test,
...
impl StructuredProcessExecutionClass {
    ...
    pub fn hard_multiplier(self) -> u64 {
        match self {
            Self::RustBuild | Self::SwiftBuild => 4,
            Self::Test | Self::Packaging => 3,
            Self::Generic => 1,
        }
    }

    pub fn hard_timeout_secs(self, soft_timeout_secs: u64, max_timeout_secs: u64) -> u64 {
        let soft = soft_timeout_secs.max(1);
        soft.saturating_mul(self.hard_multiplier())
            .min(max_timeout_secs.max(soft))
    }
}

pub fn classify_structured_process_command(
    executable: &str,
    args: &[String],
    ...
    match executable.as_str() {
        "cargo" if args.first().is_some_and(|arg| arg == "test") => {
            StructuredProcessExecutionClass::Test
        }
        "cargo" if args.first().is_some_and(|arg| arg == "package") => {
            StructuredProcessExecutionClass::Packaging
        }
        "cargo"
            if args
                .first()
                .is_some_and(|arg| matches!(arg.as_str(), "build" | "check" | "rustc")) =>
        {
            StructuredProcessExecutionClass::RustBuild
        }
        "rustc" | "rustc.exe" => StructuredProcessExecutionClass::RustBuild,
        "swift" if args.first().is_some_and(|arg| arg == "test") => {
            StructuredProcessExecutionClass::Test
        }
        "swift" if args.first().is_some_and(|arg| arg == "package") => {
            StructuredProcessExecutionClass::Packaging
        }
        "swift" if args.first().is_some_and(|arg| arg == "build") => {
            StructuredProcessExecutionClass::SwiftBuild
        }
        "swiftc" | "swiftc.exe" => StructuredProcessExecutionClass::SwiftBuild,
        "xcodebuild" if has("test") || has("test-without-building") => {
            StructuredProcessExecutionClass::Test
        }
        "xcodebuild" if has("archive") || has("-archivepath") => {
            StructuredProcessExecutionClass::Packaging
        }
        "xcodebuild" => StructuredProcessExecutionClass::SwiftBuild,
        _ => StructuredProcessExecutionClass::Generic,
    }
}
pub const PROCESS_TIMEOUT_MAX_SECS: u64 = STRUCTURED_EXECUTION_TIMEOUT_MAX_SECS;

pub const SEARCH_TEXT_REQUEST_KIND: &str = "search_text";
pub const SEARCH_TEXT_PATTERN_MAX_BYTES: usize = 16 * 1024;
pub const SEARCH_TEXT_PATH_MAX_BYTES: usize = 1_024;
pub const SEARCH_TEXT_MAX_GLOBS: usize = 32;
pub const SEARCH_TEXT_GLOB_MAX_BYTES: usize = 256;
pub const SEARCH_TEXT_MAX_CONTEXT_LINES: usize = 20;
pub const SEARCH_TEXT_MAX_LIMIT: usize = 200;
pub const SEARCH_TEXT_REQUEST_MAX_BYTES: usize = 64 * 1024;

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum RunnerTextSearchPatternMode {
    Regex,
    Literal,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum RunnerTextSearchResultMode {
    Matches,
    FilesWithMatches,
    Count,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct RunnerTextSearchRequest {
    pub pattern: String,
    pub pattern_mode: RunnerTextSearchPatternMode,
    pub path: String,
    pub limit: usize,
    ...
    pub result_mode: RunnerTextSearchResultMode,
}

pub fn validate_text_search_request(request: &RunnerTextSearchRequest) -> Result<(), String> {
    if request.pattern.is_empty()
        || request.pattern.len() > SEARCH_TEXT_PATTERN_MAX_BYTES
        || request.pattern.contains('\0')
    {
        return Err("search pattern is empty, invalid, or oversized".to_string());
    }
    if request.path.is_empty()
        || request.path.len() > SEARCH_TEXT_PATH_MAX_BYTES
        || request.path.contains('\0')
    {
        return Err("search path is empty, invalid, or oversized".to_string());
    }
    ...
    {
        return Err("search path must remain project-relative".to_string());
    }
    if !(1..=SEARCH_TEXT_MAX_LIMIT).contains(&request.limit) {
        return Err("search limit is outside the supported range".to_string());
    }
    if request.context_before > SEARCH_TEXT_MAX_CONTEXT_LINES
        || request.context_after > SEARCH_TEXT_MAX_CONTEXT_LINES
    {
        return Err("search context exceeds the supported range".to_string());
    }
    for (label, globs) in [
        ("include_globs", &request.include_globs),
        ("exclude_globs", &request.exclude_globs),
    ] {
        if globs.len() > SEARCH_TEXT_MAX_GLOBS {
            return Err(format!("{label} contains too many entries"));
        }
        for glob in globs {
            if glob.is_empty()
                || glob.len() > SEARCH_TEXT_GLOB_MAX_BYTES
                || glob.chars().any(char::is_control)
                || glob.starts_with('!')
            {
    ...
    let bytes = serde_json::to_vec(request)
        .map_err(|error| format!("search request could not be encoded: {error}"))?
        .len();
    if bytes > SEARCH_TEXT_REQUEST_MAX_BYTES {
        return Err("search request exceeds the Runner payload bound".to_string());
    }
    Ok(())
}
...
mod envelope_tests {
    ...
    #[test]
    fn structured_process_classification_and_hard_deadline_are_shared() {
        let build = classify_structured_process_command(
            "/opt/toolchain/bin/cargo",
            &["build".to_string(), "--release".to_string()],
        );
        assert_eq!(build, StructuredProcessExecutionClass::RustBuild);
        assert_eq!(build.label(), "rust_build");
        assert_eq!(build.hard_timeout_secs(300, 3_600), 1_200);

        let test = classify_structured_process_command(
            "xcodebuild",
            &["test-without-building".to_string()],
        );
        assert_eq!(test, StructuredProcessExecutionClass::Test);
        assert_eq!(test.hard_timeout_secs(300, 3_600), 900);

        let generic = classify_structured_process_command("git", &["status".to_string()]);
        assert_eq!(generic, StructuredProcessExecutionClass::Generic);
        assert_eq!(generic.hard_timeout_secs(300, 3_600), 300);
        assert_eq!(build.hard_timeout_secs(300, 600), 600);
    }
