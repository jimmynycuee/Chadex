...
use crate::runner_protocol::{
    ...
    RunnerTextSearchRequest, ShellFileOpRequest, ShellJobContext,
    ...
    SEARCH_TEXT_REQUEST_KIND, SEARCH_TEXT_REQUEST_MAX_BYTES,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RunnerSearchTextOperation {
    pub cwd: Option<String>,
    pub request: RunnerTextSearchRequest,
    pub timeout_secs: u64,
}
...
pub enum RunnerOperation {
    ...
    SearchText(RunnerSearchTextOperation),
...
impl RunnerOperation {
    pub fn wire_kind(&self) -> &'static str {
        match self {
            ...
            Self::SearchText(_) => SEARCH_TEXT_REQUEST_KIND,
...
fn encode_operation(
    ...
    match operation {
        ...
        RunnerOperation::SearchText(operation) => {
            validate_direct_structured_common(operation.cwd.as_deref(), None, operation.timeout_secs)?;
            validate_text_search_request(&operation.request)?;
            let payload = serde_json::to_string(&operation.request)
                .map_err(|error| format!("could not encode search_text request: {error}"))?;
            if payload.len() > SEARCH_TEXT_REQUEST_MAX_BYTES {
                return Err("search_text request exceeds the Runner payload bound".to_string());
            }
            wire.cwd = operation.cwd;
            wire.stdin = Some(payload);
            wire.timeout_secs = operation.timeout_secs;
        }
...
fn decode_operation(wire: &RunnerRequest) -> Result<RunnerOperation, String> {
    ...
    match wire.kind.as_str() {
        ...
        kind if kind == SEARCH_TEXT_REQUEST_KIND => {
            ...
            if wire.job_id.is_some()
                ...
                || wire.script.is_some()
            {
                return Err("search_text contains incompatible execution fields".to_string());
            }
            validate_direct_structured_common(wire.cwd.as_deref(), None, wire.timeout_secs)?;
            let payload = wire
                .stdin
                .as_deref()
                .ok_or_else(|| "search_text requires JSON payload".to_string())?;
            if payload.len() > SEARCH_TEXT_REQUEST_MAX_BYTES || payload.contains('\0') {
                return Err("search_text payload is invalid or oversized".to_string());
            }
            let request = serde_json::from_str::<RunnerTextSearchRequest>(payload)
                .map_err(|error| format!("search_text payload is invalid: {error}"))?;
            validate_text_search_request(&request)?;
            Ok(RunnerOperation::SearchText(RunnerSearchTextOperation {
                cwd: wire.cwd.clone(),
                request,
                timeout_secs: wire.timeout_secs,
            }))
...
mod tests {
        ...
        let mut operations = vec![
            ...
            RunnerOperation::SearchText(RunnerSearchTextOperation {
                ...
                request: RunnerTextSearchRequest {
                    ...
                    pattern_mode: crate::runner_protocol::RunnerTextSearchPatternMode::Literal,
                    path: "src".to_string(),
                    limit: 20,
                    context_before: 1,
                    context_after: 1,
                    ...
                    result_mode: crate::runner_protocol::RunnerTextSearchResultMode::Matches,
                },
                timeout_secs: 30,
            }),
    ...
    fn representative_v2_json_shapes_keep_field_names_and_canonical_payload_placement() {
        ...
        let search = serde_json::to_value(round_trip_kind(RunnerOperation::SearchText(
            RunnerSearchTextOperation {
                cwd: Some("/repo".to_string()),
                request: RunnerTextSearchRequest {
                    pattern: "needle".to_string(),
                    pattern_mode: crate::runner_protocol::RunnerTextSearchPatternMode::Regex,
                    path: "src".to_string(),
                    limit: 7,
                    context_before: 2,
                    context_after: 3,
                    ...
                    result_mode: crate::runner_protocol::RunnerTextSearchResultMode::Count,
                },
                timeout_secs: 30,
            },
        )))
        .unwrap();
        assert_eq!(search["kind"], SEARCH_TEXT_REQUEST_KIND);
        assert_eq!(search["command"], "");
        assert!(search.get("process").is_none());
        assert!(search.get("script").is_none());
        let payload: RunnerTextSearchRequest =
            serde_json::from_str(search["stdin"].as_str().expect("search_text stdin")).unwrap();
        assert_eq!(payload.pattern, "needle");
        assert_eq!(payload.limit, 7);
        assert_eq!(
            payload.result_mode,
            crate::runner_protocol::RunnerTextSearchResultMode::Count
        );
