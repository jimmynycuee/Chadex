...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=CHADEX_RUNTIME_GIT_COMMIT");
    println!("cargo:rerun-if-env-changed=CHADEX_RUNTIME_GIT_DIRTY");
    println!("cargo:rerun-if-env-changed=CHADEX_RUNTIME_BUILT_AT");
    ...
    // CI/release callers that pin CHADEX_RUNTIME_GIT_DIRTY intentionally skip these
    // worktree dependencies and keep their deterministic identity contract.
    let git_dirty_override = env_value("CHADEX_RUNTIME_GIT_DIRTY");
    ...
    let git_commit =
        env_value("CHADEX_RUNTIME_GIT_COMMIT").unwrap_or_else(|| git_commit_from_git(&repo_root));
    // Release workflows pin CHADEX_RUNTIME_BUILT_AT explicitly. For ordinary Git
    ...
    let built_at = env_value("CHADEX_RUNTIME_BUILT_AT")
        .or_else(|| env_value("SOURCE_DATE_EPOCH"))
        .or_else(|| git_commit_timestamp_from_git(&repo_root))
        .unwrap_or_else(current_unix_timestamp);

    println!("cargo:rustc-env=CHADEX_RUNTIME_BUILD_GIT_COMMIT={git_commit}");
    println!("cargo:rustc-env=CHADEX_RUNTIME_BUILD_GIT_DIRTY={git_dirty}");
    println!("cargo:rustc-env=CHADEX_RUNTIME_BUILD_BUILT_AT={built_at}");
}
