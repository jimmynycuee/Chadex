...
fn text_search_request(pattern: &str, path: &str, limit: usize) -> RunnerTextSearchRequest {
    RunnerTextSearchRequest {
        pattern: pattern.to_string(),
        pattern_mode: RunnerTextSearchPatternMode::Literal,
        path: path.to_string(),
        limit,
        context_before: 0,
        context_after: 0,
        ...
        result_mode: RunnerTextSearchResultMode::Matches,
    }
}

#[test]
fn native_text_search_literal_matches_without_shell_wrapper() {
    ...
    .unwrap();
    let result = run_native_text_search(
        root.path(),
        text_search_request("literal.a", "sample.txt", 20),
    );
    assert_eq!(result.execution_state, ShellCommandExecutionState::Completed);
    assert_eq!(result.result.exit_code, Some(0));
    let stdout = result.result.stdout.unwrap_or_default();
    assert!(
        stdout.starts_with("{\"webcodex_search\":"),
        "typed search must keep the established parser marker: {stdout:?}"
    );
    assert!(stdout.contains("literal.a"));
    assert!(!stdout.contains("literalXa"));
}

#[test]
fn native_text_search_limit_keeps_one_probe_record_and_stops_early() {
    let root = tempfile::tempdir().unwrap();
    std::fs::write(
    ...
    let result = run_native_text_search(
        root.path(),
        text_search_request("needle", "many.txt", 2),
    );
    assert_eq!(result.execution_state, ShellCommandExecutionState::Completed);
    assert_eq!(result.result.exit_code, Some(141));
    let stdout = result.result.stdout.unwrap_or_default();
    let payload_lines = stdout.lines().skip(1).count();
    assert_eq!(
        payload_lines, 3,
        "limit=2 must retain the established +1 truncation probe"
    );
}

#[test]
fn native_text_search_rg_argv_preserves_globs_modes_and_protected_excludes() {
    let request = RunnerTextSearchRequest {
        ...
        result_mode: RunnerTextSearchResultMode::Count,
    };
    let args = search_text_rg_args(&request);
    assert!(args.iter().any(|arg| arg == "--fixed-strings"));
    assert!(args.iter().any(|arg| arg == "--count"));
    assert!(args.windows(2).any(|pair| pair == ["--glob", "*.rs"]));
    assert!(
        args.windows(2)
            .any(|pair| pair == ["--glob", "!generated/**"])
    );
    assert!(
        args.windows(2)
            .any(|pair| pair == ["--glob", "!**/.git/**"])
    );
...
#[cfg(unix)]
#[test]
fn native_text_search_rejects_symlink_escape_from_project_root() {
    ...
    let result = run_native_text_search(
        root.path(),
        text_search_request("needle", "escape/secret.txt", 20),
    );
    assert_eq!(
    ...
    assert!(
        result
            ...
            .error
            .as_deref()
            .is_some_and(|error| error.contains("escaped the project root"))
    );
}
...
#[test]
fn short_command_completion_poll_stays_low_latency_and_backs_off() {
    assert_eq!(
        shell_command_completion_poll(Duration::ZERO),
        Duration::from_millis(1)
    );
    assert_eq!(
        shell_command_completion_poll(SHELL_COMMAND_FAST_POLL_WINDOW - Duration::from_millis(1)),
        Duration::from_millis(1)
    );
    assert_eq!(
        shell_command_completion_poll(SHELL_COMMAND_FAST_POLL_WINDOW),
        Duration::from_millis(5)
    );
}

#[test]
fn phase13_command_classes_use_bounded_adaptive_budgets() {
    assert_eq!(
        classify_process_command("cargo", &["build".into(), "--release".into()]),
        CommandExecutionClass::RustBuild
    );
    assert_eq!(
        classify_process_command("cargo", &["test".into(), "--workspace".into()]),
        CommandExecutionClass::Test
    );
    assert_eq!(
        classify_process_command("xcodebuild", &["archive".into()]),
        CommandExecutionClass::Packaging
    );
    assert_eq!(
        classify_process_command("git", &["status".into()]),
        CommandExecutionClass::Generic
    );
    assert_eq!(
        classify_shell_command(r#""$CHADEX_CARGO" build --locked --profile dogfood --bins"#),
        CommandExecutionClass::RustBuild
    );

    let build = adaptive_timeout_budget(CommandExecutionClass::RustBuild, 300, 3_600);
    assert_eq!(build.soft, Duration::from_secs(300));
    assert_eq!(build.grace, Duration::from_secs(60));
    assert_eq!(build.hard, Duration::from_secs(1_200));

    let generic = adaptive_timeout_budget(CommandExecutionClass::Generic, 300, 3_600);
    assert_eq!(generic.soft, Duration::from_secs(300));
    assert_eq!(generic.hard, Duration::from_secs(300));
}

#[cfg(unix)]
#[test]
fn phase13_silent_cpu_progress_can_cross_soft_timeout() {
    let cwd = tempfile::tempdir().unwrap();
    let mut policy = unrestricted_policy();
    policy.max_timeout_secs = 5;
    ...
    assert_eq!(result.result.exit_code, Some(0));
    assert!(
        result.result.duration_ms.unwrap_or_default() >= 2_000,
        "recognized build should have survived beyond its 1s soft timeout"
    );
}

#[cfg(unix)]
#[test]
fn phase13_stalled_recognized_build_is_still_terminated() {
    let cwd = tempfile::tempdir().unwrap();
    let mut policy = unrestricted_policy();
    policy.max_timeout_secs = 5;
    let result = run_shell_impl(
    ...
    assert_eq!(result.result.error.as_deref(), Some("timeout_stalled"));
    assert!(
        result.result.duration_ms.unwrap_or_default() < 4_000,
        "stalled process should terminate well before the 4s hard bound"
    );
}

#[cfg(unix)]
#[test]
fn phase13_build_failure_is_not_masked_by_adaptive_timeout() {
    let result = run_shell_impl(
        &unrestricted_policy(),
        &ShellConfig::default(),
        None,
    ...
    assert_eq!(result.result.exit_code, Some(17));
    assert_ne!(
        result.result.error.as_deref(),
        Some("timeout_with_progress")
    );
    assert_ne!(result.result.error.as_deref(), Some("timeout_stalled"));
}

#[cfg(unix)]
#[test]
fn phase13_test_failure_is_not_masked_by_adaptive_timeout() {
    let result = run_shell_impl(
        &unrestricted_policy(),
        &ShellConfig::default(),
        None,
...
#[cfg(unix)]
#[test]
#[ignore = "real Phase 13 dogfood optimized build regression"]
fn phase13_dogfood_optimized_build_uses_300s_soft_timeout_without_destructive_restart() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../..")
        .canonicalize()
        .expect("repo root");
    let cargo = std::env::var("CHADEX_CARGO")
        .or_else(|_| std::env::var("CARGO"))
        .expect("CHADEX_CARGO or CARGO must identify the Rust toolchain");
    let manifest = repo_root.join("chadex-runtime/Cargo.toml");
    let target = repo_root.join("runtime-engine/target");
    let command = format!(
        "{} build --locked --manifest-path {} --target-dir {} --profile dogfood --bins",
        shell_quote(&cargo),
        shell_quote(manifest.to_string_lossy().as_ref()),
        shell_quote(target.to_string_lossy().as_ref()),
    );
    assert_eq!(
    ...
    let incremental = target.join("dogfood/incremental");
    let incremental_existed_before = incremental.exists();
    let mut policy = unrestricted_policy();
    policy.max_timeout_secs = 3_600;
    let result = run_shell_impl(
        &policy,
        &ShellConfig::default(),
        None,
    ...
    let duration_ms = result.result.duration_ms.unwrap_or_default();
    eprintln!(
        "PHASE13_DOGFOOD duration_ms={duration_ms} soft_timeout_ms=300000 execution_state={:?} exit_code={:?}",
        result.execution_state, result.result.exit_code
    );
    ...
    assert_eq!(result.result.exit_code, Some(0));
    if incremental_existed_before {
        assert!(
            incremental.exists(),
            "Phase 13 must not clear existing Cargo incremental state"
        );
    }
}
