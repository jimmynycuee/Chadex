...
pub(super) fn run_git_bounded(
    ...
    run_git_bounded_with_program_env("git", path, args, timeout, shutdown, &[])
}

/// Run Git with a deliberately scoped environment. The only current caller is
/// the dirty-worktree snapshot path, which supplies a temporary
/// `GIT_INDEX_FILE`; the user's real index is never opened for write.
pub(super) fn run_git_bounded_with_env(
    path: &Path,
    args: &[&str],
    timeout: Duration,
    shutdown: Option<&AtomicBool>,
    ...
    run_git_bounded_with_program_env("git", path, args, timeout, shutdown, env)
}
...
pub(super) fn run_git_bounded_with_program(
    ...
    run_git_bounded_with_program_env(program, path, args, timeout, shutdown, &[])
}

fn run_git_bounded_with_program_env(
    program: &str,
    path: &Path,
    args: &[&str],
    timeout: Duration,
    ...
    let deadline = observation_started + timeout;
    let status = loop {
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                // Worktree setup runs several short Git probes in sequence.
                // A fixed 10 ms sleep charged each one a scheduling floor.
                // Bound fast polling to 25 ms; long Git commands retain the
                // original cadence and all timeout/tree-cleanup semantics.
                let cadence = if observation_started.elapsed() < Duration::from_millis(25) {
                    Duration::from_millis(1)
                } else {
                    Duration::from_millis(10)
                };
                thread::sleep(cadence.min(deadline.saturating_duration_since(Instant::now())));
            }
            Err(error) => {
                let _ = terminate_project_git_tree(
                    &mut child,
