...
use super::catalog::{
    ...
    project_root_fingerprint, project_wire_kind, run_git_bounded, run_git_bounded_with_env,
...
/// Materialize the current tracked, staged, unstaged, and non-ignored
/// untracked files into an anonymous commit without touching the source index,
/// HEAD, or stash. A fixed commit identity/date makes retries for the same
/// source tree converge to the same object id.
fn create_dirty_snapshot(source: &Path) -> Result<String, &'static str> {
    let index_path = std::env::temp_dir().join(format!(
        "chadex-execution-index-{}.index",
        uuid::Uuid::new_v4().simple()
    ));
    let index_string = index_path.to_string_lossy().into_owned();
    let env = [
        ("GIT_INDEX_FILE", index_string.as_str()),
    ...
    let run = |args: &[&str]| {
        run_git_bounded_with_env(source, args, MANAGED_WORKTREE_GIT_TIMEOUT, None, &env)
            .map_err(|_| "dirty_snapshot_git_failed")
    };
    let result = (|| {
        let output = run(&["read-tree", "HEAD"])?;
        if !output.status.success() {
            return Err("dirty_snapshot_read_tree_failed");
        }
        let output = run(&["add", "-A", "--", "."])?;
        if !output.status.success() {
            return Err("dirty_snapshot_add_failed");
        }
        let output = run(&["write-tree"])?;
        if !output.status.success() || output.stdout_capped {
            return Err("dirty_snapshot_write_tree_failed");
        }
        let tree = String::from_utf8(output.stdout)
            .map_err(|_| "dirty_snapshot_write_tree_failed")?
            .trim()
            .to_string();
        ...
        if !output.status.success() || output.stdout_capped {
            return Err("dirty_snapshot_commit_failed");
        }
        let commit = String::from_utf8(output.stdout)
            .map_err(|_| "dirty_snapshot_commit_failed")?
            .trim()
            .to_string();
...
fn choose_managed_worktree_root(
    ...
    if let Some(managed_root) = policy.managed_worktree_root.as_ref() {
        if managed_root.as_os_str().is_empty()
            || !managed_root.is_absolute()
            || webcodex_runner_config::paths::project_path_has_parent_traversal(managed_root)
            || webcodex_runner_config::paths::path_is_within(managed_root, source)
        {
            return Err("managed_worktree_root_unavailable");
        }
        return Ok(managed_root.clone());
    }
    let mut roots =
...
pub(crate) fn handle_prepare_managed_worktree_operation(
    ...
    if !(4..=5).contains(&payload.len()) {
        return managed_worktree_error(start, "invalid_request", false, None, None, None);
    }
    if payload.keys().any(|key| {
        !matches!(
            key.as_str(),
            "path" | "base_ref" | "operation_id" | "resume_project_id" | "include_dirty_snapshot"
        )
    }) {
        return managed_worktree_error(start, "invalid_request", false, None, None, None);
    }
    ...
    let include_dirty_snapshot = match payload.get("include_dirty_snapshot") {
        None => false,
    ...
    let source_allow_patch = source_projects
        .iter()
        .find(|project| project.id == source_project_id)
        .map(|project| project.allow_patch)
        .unwrap_or(false);
    let snapshot_created = include_dirty_snapshot && source_dirty;
    let base_sha = if snapshot_created {
        match create_dirty_snapshot(&source_root) {
            Ok(sha) => sha,
            Err(error) => {
                return managed_worktree_error(
                    start,
    ...
    if webcodex_runner_config::paths::path_is_within(&managed_root, &source_root)
        || validate_project_path_policy(policy, &managed_root).is_err()
    {
        return managed_worktree_error(
            start,
...
mod phase14a_tests {
    ...
    #[test]
    fn dedicated_managed_root_works_with_exact_source_authority() {
        let temp = tempfile::tempdir().unwrap();
        let source = temp.path().join("source");
        std::fs::create_dir_all(&source).unwrap();
        let source = source.canonicalize().unwrap();
        let managed = temp.path().join("chadex-state").join("managed-worktrees");
        let policy = RunnerPolicy {
            allowed_roots: vec![source.clone(), managed.clone()],
            managed_worktree_root: Some(managed.clone()),
            ..RunnerPolicy::default()
        };

        let chosen = choose_managed_worktree_root(&policy, &source).unwrap();
        assert_eq!(chosen, managed);
        assert!(!webcodex_runner_config::paths::path_is_within(
            &chosen, &source
        ));
    }

    #[test]
    fn dedicated_managed_root_rejects_destination_inside_source() {
        let temp = tempfile::tempdir().unwrap();
        let source = temp.path().join("source");
        std::fs::create_dir_all(&source).unwrap();
        let source = source.canonicalize().unwrap();
        let managed = source.join("managed-worktrees");
        let policy = RunnerPolicy {
            allowed_roots: vec![source.clone()],
            managed_worktree_root: Some(managed),
            ..RunnerPolicy::default()
        };

        assert_eq!(
            choose_managed_worktree_root(&policy, &source),
            Err("managed_worktree_root_unavailable")
        );
    }
}
