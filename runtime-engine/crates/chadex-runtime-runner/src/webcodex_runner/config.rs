...
pub(crate) struct RunnerPolicy {
    ...
    /// Chadex-owned destination root for isolated managed worktrees. This is
    /// separate from source-project selection so an exact project root need not
    /// grant its parent directory as general Runner authority.
    #[serde(default)]
    pub(crate) managed_worktree_root: Option<PathBuf>,
...
pub(crate) fn load_config(path: &Path) -> Result<RunnerConfig, String> {
    ...
    if let Some(managed_root) = cfg.policy.managed_worktree_root.as_ref() {
        ...
        {
            return Err(
                "policy.managed_worktree_root must be a non-empty absolute path without parent traversal"
                    .to_string(),
            );
        }
        if !effective.iter().any(|root| root == managed_root) {
            effective.push(managed_root.clone());
        }
    }
