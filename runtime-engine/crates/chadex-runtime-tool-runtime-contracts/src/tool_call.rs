...
where
    D: serde::Deserializer<'de>,
{
    let revision = Option::<u64>::deserialize(deserializer)?;
    if revision.is_some_and(|revision| !(1..=9_007_199_254_740_991_u64).contains(&revision)) {
...
#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ChadexTaskPreplan {
    pub estimated_file_count: usize,
    pub estimated_subsystem_count: usize,
    pub estimated_language_count: usize,
    pub validation_domain_count: usize,
    #[serde(default)]
    pub cross_runtime_boundary: bool,
    #[serde(default)]
    pub stateful_or_schema_change: bool,
    #[serde(default)]
    pub concurrency_or_security_sensitive: bool,
    #[serde(default)]
    pub graphify_informed: bool,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ChadexTaskBatchItem {
    #[serde(default)]
    pub task_id: Option<String>,
    pub goal: String,
    #[serde(default)]
    pub preplan: Option<ChadexTaskPreplan>,
    #[serde(default)]
    pub package_count: Option<usize>,
    pub steps: Vec<ChadexTaskStep>,
    #[serde(default)]
    pub policy: Option<ChadexTaskPolicy>,
    #[serde(default)]
    pub acceptance: Option<ChadexTaskAcceptance>,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ChadexTaskAcceptance {
    #[serde(default)]
    pub require_validation_success: Option<bool>,
    #[serde(default)]
...
pub enum ToolCall {
    ...
    /// Execute a deterministic bounded multi-step Chadex task plan. No LLM is
    /// invoked by Runtime; each step is routed through ordinary first-class tools.
    ExecuteTask {
        project: String,
        #[serde(default)]
    ...
    /// Execute two to four deterministic Chadex task plans concurrently through
    /// the same bounded scheduler. Each item retains its own task lifecycle.
    ExecuteTasks {
        project: String,
        tasks: Vec<ChadexTaskBatchItem>,
    },

    /// Read the current process-local state of one exact deterministic task.
    ObserveTask {
        project: String,
        task_id: String,
    },

    /// Cooperatively cancel one exact deterministic task between bounded steps.
    CancelTask {
        project: String,
        task_id: String,
    },

    /// Recover deterministic Chadex task state persisted across a runtime restart.
    TaskRecovery {
        project: String,
        action: String,
        #[serde(default)]
    ...
    ReadFiles {
        ...
        #[serde(default)]
        with_line_numbers: Option<bool>,
        /// Opt in to model-facing snapshot handles for successful complete
        /// reads. Partial reads still receive a revision automatically because
        /// a safe continuation cannot be represented without one.
        #[serde(default)]
...
impl ToolCall {
    ...
    pub fn tool_name(&self) -> &'static str {
        match self {
            ...
            Self::ExecuteTasks { .. } => "execute_tasks",
            Self::ObserveTask { .. } => "observe_task",
            Self::CancelTask { .. } => "cancel_task",
            Self::TaskRecovery { .. } => "task_recovery",
    ...
    pub fn session_id(&self) -> Option<&str> {
        match self {
            ...
            Self::ExecuteTask { session_id, .. } => session_id.as_deref(),
    ...
    pub fn project(&self) -> Option<&str> {
        match self {
            ...
            | Self::ExecuteTask { project, .. }
            | Self::ExecuteTasks { project, .. }
            | Self::ObserveTask { project, .. }
            | Self::CancelTask { project, .. }
            | Self::TaskRecovery { project, .. }
