...
use super::tool_call::{
    ChadexTaskStep, ComputerControlToolCall, ComputerObserveToolCall, ToolCall,
};
    ...
    pub fn session_log_arguments(&self) -> Value {
        match self {
            ...
            Self::ExecuteTask {
                ...
                "project": project,
                "task_id_present": task_id.is_some(),
                "goal_bytes": goal.len(),
                "preplan": preplan,
                "package_count": package_count,
                "step_count": steps.len(),
                "step_kinds": steps.iter().map(|step| match step {
                ...
                "acceptance_present": acceptance.is_some(),
                "session_id": session_id,
            }),
            Self::ExecuteTasks { project, tasks } => serde_json::json!({
                "project": project,
                "task_count": tasks.len(),
                "task_ids_present": tasks.iter().filter(|task| task.task_id.is_some()).count(),
                "step_counts": tasks.iter().map(|task| task.steps.len()).collect::<Vec<_>>(),
                "session_ids_present": tasks.iter().filter(|task| task.session_id.is_some()).count(),
            }),
            Self::ObserveTask { project, task_id } | Self::CancelTask { project, task_id } => {
                serde_json::json!({
                    "project": project,
                    "task_id": task_id,
                })
