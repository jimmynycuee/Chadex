...
const MAX_HELPER_INGRESS_PRE_BACKEND_US: u64 = 30_000_000;

fn helper_ingress_timing(req: &Request) -> Option<(u64, u64)> {
    let started_unix_ns = request_header(req, HELPER_INGRESS_STARTED_UNIX_NS_HEADER)?
        .parse()
        .ok()?;
    let pre_backend_us = request_header(req, HELPER_INGRESS_PRE_BACKEND_US_HEADER)?
        .parse()
        .ok()?;
    if started_unix_ns == 0 || pre_backend_us > MAX_HELPER_INGRESS_PRE_BACKEND_US {
        return None;
    }
    Some((started_unix_ns, pre_backend_us))
}
...
pub async fn mcp_post(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    // Always generated here, never trusted from a caller. This joins ingress /
    // client timings to the existing Server -> Runner lifecycle trace without
    // enabling payload retention or changing authorization / dispatch.
    res.headers_mut().insert(
        "x-chadex-trace-id",
        guard
            .correlation_trace_id()
            .parse()
            .expect("generated UUID header"),
    );
    if let Some((started_unix_ns, pre_backend_us)) = helper_ingress_timing(req) {
        guard.set_helper_ingress_timing(started_unix_ns, pre_backend_us);
    }
    guard.received();
    guard.phase("mcp_auth_validation_started", None);
    ...
    // Input size only. The request arguments are never written into Phase 15
    // metadata, even when a caller includes source or prompt text.
    guard.phase("mcp_request_decoded", estimate_json_bytes(&request.params));
    ...
    for link in &tool_correlation.workflow_sessions {
        guard.link_workflow_session(&link.session_id);
    }
