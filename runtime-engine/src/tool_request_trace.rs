...
static TRACE_PROCESS_ORIGIN: OnceLock<Instant> = OnceLock::new();
...
/// Emit a bounded, content-free subphase for the currently scoped request.
/// The duration is measured by the producer's monotonic clock; the event wall
/// timestamp is only for joining independently produced events.
pub(crate) fn record_active_phase(
    name: &'static str,
    duration: Duration,
    bytes: Option<usize>,
    ...
    let mut event = base_event(&trace_id, name);
    merge_event_fields(
        &mut event,
        json!({
            "duration_ns": u64::try_from(duration.as_nanos()).ok(),
            "bytes": bytes,
            "item_count": item_count,
        }),
    );
    enqueue_metadata_event(&trace_id, name, event);
}
...
fn base_event(trace_id: &str, event: &str) -> Value {
    ...
    json!({
        ...
        "wall_unix_ns": SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
            .ok().and_then(|elapsed| u64::try_from(elapsed.as_nanos()).ok()),
        "process_elapsed_ns": u64::try_from(TRACE_PROCESS_ORIGIN.get_or_init(Instant::now)
            .elapsed().as_nanos()).ok(),
        "event": event,
        "server_trace_id": trace_id,
...
pub(crate) fn record_runner_request_enqueued<T: Serialize>(
    ...
    tracing::info!(
        ...
        runner_client_sha256 = %sha256_hex(client_id.as_bytes()),
        runner_request_kind = kind,
        runner_job_id = job_id.unwrap_or("-"),
        runner_agent_instance_sha256 = runner_instance_id.map(|id| sha256_hex(id.as_bytes())).unwrap_or_default(),
    ...
    if tool_request_trace_enabled() {
        ...
        merge_event_fields(
            ...
            json!({
                ...
                "runner_agent_instance_sha256": runner_instance_id.map(|id| sha256_hex(id.as_bytes())),
...
/// Registry lease/poll has committed dispatch. This is distinct from enqueue
/// and does not claim that a command has started on the Runner host.
pub(crate) fn record_runner_request_dispatched(request_id: &str) {
    let Some(correlation) = lookup_request_correlation(request_id) else {
        return;
    };
    let mut event = base_event(&correlation.trace_id, "tool_runner_request_dispatched");
    merge_event_fields(
        &mut event,
        json!({
            "runner_request_id": request_id,
            "runner_request_kind": correlation.runner_kind,
        }),
    );
    enqueue_metadata_event(&correlation.trace_id, "runner_request_dispatched", event);
}

/// Authoritative accepted result and Runner-reported execution duration. The
/// latter is never inferred from Server-to-Runner elapsed time.
pub(crate) fn record_runner_result_metrics(request_id: &str, duration_ms: Option<u64>) {
    let Some(correlation) = lookup_request_correlation(request_id) else {
        return;
    };
    ...
    merge_event_fields(
        ...
        json!({
            "runner_request_id": request_id,
            "runner_request_kind": correlation.runner_kind,
            "runner_reported_duration_ms": duration_ms,
        }),
    );
...
pub struct ToolRequestLifecycle {
    ...
    helper_ingress_started_unix_ns: Option<u64>,
    helper_ingress_pre_backend_us: Option<u64>,
...
fn safe_method_label(method: &str) -> &'static str {
    match method {
        "POST /mcp" => "POST /mcp",
        "POST /api/tools/call" => "POST /api/tools/call",
...
fn safe_tool_label(tool_name: Option<&str>) -> Option<&str> {
    tool_name.filter(|name| crate::tool_runtime::metadata::lookup_tool_metadata(name).is_some())
}
...
impl ToolRequestLifecycle {
    pub fn new(
        ...
        Self {
            ...
            client_window: None,
            app_call_id: None,
            helper_ingress_started_unix_ns: None,
            helper_ingress_pre_backend_us: None,
    ...
    /// Attach numeric timing produced by Chadex's local ingress proxy. These
    /// values are diagnostics only: they never participate in authorization,
    /// routing, Session identity, or tool execution.
    pub(crate) fn set_helper_ingress_timing(
        &mut self,
        started_unix_ns: u64,
        pre_backend_us: u64,
    ) {
        self.helper_ingress_started_unix_ns = Some(started_unix_ns);
        self.helper_ingress_pre_backend_us = Some(pre_backend_us);
    }
    ...
    fn log_with_duration(
        ...
        tracing::info!(
            ...
            method = safe_method_label(&self.method),
            tool_name = safe_tool_label(self.tool_name.as_deref()).unwrap_or("-"),
        ...
        if self.enabled() {
            ...
            merge_event_fields(
                ...
                json!({
                    ...
                    "method": safe_method_label(&self.method),
                    "tool_name": safe_tool_label(self.tool_name.as_deref()),
                    ...
                    "helper_ingress_started_unix_ns": self.helper_ingress_started_unix_ns,
                    "helper_ingress_pre_backend_us": self.helper_ingress_pre_backend_us,
    ...
    /// One content-free boundary in the existing request trace. The name is
    /// static and only numeric observations may be attached; caller data never
    /// enters the trace writer through this method.
    pub(crate) fn phase(&self, name: &'static str, bytes: Option<usize>) {
        if !self.enabled() {
            return;
        }
    ...
    pub(crate) fn link_workflow_session(&self, session_id: &str) {
        ...
        let mut event = base_event(&self.trace_id, "tool_workflow_session_linked");
        merge_event_fields(
            &mut event,
            json!({
                "workflow_session_sha256": sha256_hex(session_id.as_bytes()),
                "request_elapsed_ns": u64::try_from(self.started.elapsed().as_nanos()).ok(),
            }),
        );
        enqueue_metadata_event(&self.trace_id, "workflow_session_linked", event);
    }
...
mod tests {
    ...
    #[test]
    fn metadata_labels_reject_unrecognized_caller_content() {
        assert_eq!(safe_method_label("private-prompt-text"), "unrecognized");
        assert!(safe_tool_label(Some("private-source-text")).is_none());
        assert_eq!(safe_tool_label(Some("read_files")), Some("read_files"));
    }

    #[test]
    fn metadata_lifecycle_persists_only_safe_boundaries() {
        let temp = tempfile::tempdir().unwrap();
        let mut env = crate::test_support::TestEnvGuard::new();
        ...
        guard.set_helper_ingress_timing(1_790_000_000_000_000_000, 2_500);
        guard.capture_payload(
            "raw_arguments",
            &json!({"source": "private-source-sentinel"}),
        );
        guard.received();
        guard.phase("mcp_auth_validation_finished", None);
        ...
        assert!(raw.contains("mcp_auth_validation_finished"));
        assert!(raw.contains("request_elapsed_ns"));
        assert!(raw.contains("wall_unix_ns"));
        ...
        assert!(!temp
            .path()
            .join(guard.correlation_trace_id())
            .join("payloads")
            .exists());
    }
    ...
    async fn runner_payload_serialization_is_full_trace_only() {
        ...
        .await;
        record_runner_request_dispatched(&metadata_request_id);
        record_runner_result_metrics(&metadata_request_id, Some(17));
        flush_full_trace_writer();
        let metadata_events =
            fs::read_to_string(temp.path().join(&metadata_trace_id).join("events.jsonl")).unwrap();
        assert!(metadata_events.contains("tool_runner_request_enqueued"));
        assert!(metadata_events.contains("tool_runner_request_dispatched"));
        assert!(metadata_events.contains("tool_runner_result_accepted"));
        assert!(metadata_events.contains("\"runner_reported_duration_ms\":17"));
        assert!(!metadata_events.contains("\"runner_client_id\":"));
        assert!(metadata_events.contains("\"runner_client_sha256\":"));
        assert!(!temp
            .path()
            .join(&metadata_trace_id)
            .join("payloads")
            .exists());
