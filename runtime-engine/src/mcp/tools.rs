...
fn is_stateless_operator_extension(target: &str) -> bool {
    // Cache only immutable compiled contract membership, never caller scopes,
    // admission decisions, Runner state, or tool results. Building every schema
    // for a name lookup is especially costly during suggested-call projection.
...
#[test]
fn extension_membership_matches_canonical_contracts_and_rejects_other_surfaces() {
    let expected: HashSet<_> = crate::tool_runtime::stateless_operator_extension_tool_specs()
        .into_iter()
        .map(|spec| spec.name)
        .collect();
    for name in expected.iter().map(String::as_str).chain([
        "read_files",
        "runtime_status",
        "call_runtime_tool",
        "goal_plan_state",
        ...
        assert_eq!(
            is_stateless_operator_extension(name),
...
fn adaptive_runtime_gateway_tool_spec() -> ToolSpec {
    ToolSpec {
        ...
        description: "Gateway for admitted runtime tools not on the initial direct surface. Discover unknown contracts with tool_manifest. Routing never changes canonical validation, authority, permissions, Runner capability, Session policy, or tool effects.".to_string(),
        input_schema: json!({
            "type": "object",
            "properties": {
                "tool": {
                    ...
                    "minLength": 1,
                    "maxLength": 128,
                    "description": "Exact admitted runtime tool name. Prefer the direct callable when one is exposed; otherwise use this gateway."
                },
                "arguments": {
                    "type": "object",
        ...
        output_schema: json!({
            "type": "object",
            "description": "Result from the selected runtime tool.",
            "additionalProperties": true
        }),
        annotations: json!({
            "readOnlyHint": false,
...
fn mcp_adaptive_runtime_gateway_target_route(
    ...
    if stateless_2026 && is_stateless_operator_extension(target) {
...
fn mcp_suggested_tool_call_route(target: &str, stateless_2026: bool) -> SuggestedToolCallRoute {
    ...
    let operator_extension_admitted = stateless_2026 && is_stateless_operator_extension(target);
...
fn compact_mcp_common_input_descriptions(schema: &mut Value) {
    ...
    for (field, description) in [
        ...
        (
            ...
            "Retained ACK-required Session message ids. ACK does not resolve messages or grant authority.",
        ),
        (
            "session_message_resolution",
            "Resolve one handled non-todo message in the exact recording Session.",
        ),
        (
            "context_request",
            "Optional bounded post-tool context keys; grants no authority.",
        ),
        (
            "ack_session_context_revision",
            "Optional retained Session context revision; invocation metadata only.",
        ),
    ] {
        let Some(property) = properties.get_mut(field).and_then(Value::as_object_mut) else {
            continue;
        };
        property.insert(
...
fn compact_mcp_tool_list_input_descriptions(payload: &mut Value) {
    let Some(tools) = payload.get_mut("tools").and_then(Value::as_array_mut) else {
        return;
    };
    for tool in tools {
        ...
        compact_mcp_common_input_descriptions(input_schema);
    }
}

fn compact_mcp_app_tool_description(spec: &mut ToolSpec) {
    let description = match spec.name.as_str() {
        "goal_plan_state" => "App-only authorized Goal Plan state read.",
        "work_result_state" => "App-only authorized Work Result refresh read.",
        "changes_file_diff" => {
            "App-only bounded read from an authorized frozen Final Changes snapshot."
        }
        "agent_wait_state" => "App-only read of one caller-owned durable AgentWait.",
        "agent_continuation_bind" => {
            "App-only bind of this View to one authorized durable Agent endpoint generation."
        }
        "agent_continuation_recover_endpoint" => {
            "App-only idempotent recovery of one authorized expired Agent endpoint."
        }
        "agent_continuation_state" => {
            "App-only heartbeat and continuation-state refresh for the current binding."
        }
        "agent_continuation_wake_acquire" => {
            "App-only acquire of one durable Agent wake claim."
        }
        "agent_continuation_wake_prepare" => {
            "App-only pre-dispatch fence for one Agent wake attempt."
        }
        "agent_continuation_wake_finish" => {
            "App-only record of Agent wake dispatch acceptance or delivery uncertainty."
        }
        "agent_continuation_unbind" => {
            "App-only best-effort withdrawal of the current View binding."
        }
        _ => return,
    };
    spec.description.clear();
    spec.description.push_str(description);
}

fn compact_mcp_direct_tool_description(spec: &mut ToolSpec) {
    if !is_adaptive_runtime_direct_tool(&spec.name) {
        return;
    }
    let Some(description) = crate::tool_runtime::tool_definition::lookup_tool_definition(&spec.name)
        .and_then(|definition| definition.gpt_action_description())
        .filter(|description| description.len() < spec.description.len())
    else {
        return;
    };
    spec.description.clear();
...
fn mcp_tool_spec_json(mut spec: ToolSpec, compact: bool, app_enabled: bool) -> Value {
    ...
    if compact {
        compact_mcp_app_tool_description(&mut spec);
        compact_mcp_direct_tool_description(&mut spec);
        compact_mcp_common_input_descriptions(&mut spec.input_schema);
    }
...
pub(super) async fn handle_list(
    ...
    if compact_schemas {
        // Stateless recorder/context fields are injected after individual ToolSpecs are
        // projected. Re-compact their repeated descriptions at the final host boundary.
        compact_mcp_tool_list_input_descriptions(&mut result);
    }
...
pub(super) async fn handle_call(
    ...
    let protocol_extension_admitted =
        stateless_2026 && is_stateless_operator_extension(&params.name);
