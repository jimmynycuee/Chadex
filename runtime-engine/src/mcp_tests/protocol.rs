...
async fn mcp_stateless_tools_list_uses_2026_result_shape() {
    ...
    match outcome {
        McpOutcome::Ok(value) => {
            ...
            assert!(description.contains("ACK-required Session message"));
            assert!(description.contains("does not resolve"));
            assert!(description.contains("grant authority"));
            ...
            assert!(resolution_description.contains("handled non-todo message"));
            assert!(resolution_description.contains("recording Session"));
            ...
            assert!(request_description.contains("bounded post-tool context"));
            ...
            assert!(context_ack_description.contains("Session context revision"));
            assert!(context_ack_description.contains("invocation metadata"));
        }
