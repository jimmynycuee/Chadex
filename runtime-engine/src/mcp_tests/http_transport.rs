...
#[tokio::test]
async fn mcp_trace_header_is_generated_per_request_and_never_echoes_caller() {
    let config = test_config(Some("secret"));
    let (_tmp, db) = test_db();
    let service = Service::new(build_test_router(config, db, Arc::new(test_runtime())));
    let mut seen = std::collections::HashSet::new();
    for id in [1, 2] {
        let mut response = TestClient::post("http://localhost/mcp")
            .bearer_auth("secret")
            .add_header("x-chadex-trace-id", "caller-controlled", true)
            .json(&json!({"jsonrpc":"2.0", "id":id, "method":"tools/list", "params":{}}))
            .send(&service).await;
        let trace = response.headers().get("x-chadex-trace-id").unwrap().to_str().unwrap().to_string();
        assert!(uuid::Uuid::parse_str(&trace).is_ok());
        assert!(seen.insert(trace));
