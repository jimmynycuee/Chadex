...
#[tokio::test]
async fn phase15_metadata_dogfood_preserves_three_api_tool_responses() {
    ...
    for tool in ["list_tools", "runtime_status", "tool_manifest"] {
        let (status, response) =
            http_tool_call(&service, json!({"tool": tool, "params": {}})).await;
        assert_eq!(status, StatusCode::OK, "{tool}: {response}");
    ...
    for entry in std::fs::read_dir(trace_root.path()).unwrap().flatten() {
        ...
        assert!(raw.contains("api_tool_request_received"));
        assert!(raw.contains("api_tool_dispatch_started"));
        assert!(raw.contains("api_tool_handler_returned"));
        assert!(!raw.contains("tool_trace_payload_captured"));
        count += 1;
        if let Ok(output) = std::env::var("CHADEX_PHASE15_DOGFOOD_DIR") {
            let destination = std::path::Path::new(&output).join(entry.file_name());
            std::fs::create_dir_all(&destination).unwrap();
            std::fs::copy(&path, destination.join("events.jsonl")).unwrap();
        }
    }
    assert_eq!(count, 3);
}
...
#[tokio::test]
async fn phase15_metadata_overhead_smoke() {
    let trace_root = tempfile::tempdir().unwrap();
    let mut env = crate::test_support::TestEnvGuard::new();
    env.set(
        "WEBCODEX_TOOL_REQUEST_TRACE_DIR",
    ...
    let (_tmp, service) = phase2_service();
    for (tool, rounds) in [("runtime_status", 32), ("list_tools", 16)] {
        let request = json!({"tool": tool, "params": {}});
        let mut off = Vec::new();
        let mut metadata = Vec::new();
        for round in 0..rounds {
            // Reverse order on alternate rounds to limit warmup/order bias.
            ...
            for mode in modes {
                env.set("WEBCODEX_TOOL_REQUEST_TRACE", mode);
        ...
        println!("PHASE15_OVERHEAD api_{tool} n={rounds} off_median_us={} metadata_median_us={} off_p95_us={} metadata_p95_us={}",
            off[off.len()/2], metadata[metadata.len()/2], off[(rounds*95/100) as usize], metadata[(rounds*95/100) as usize]);
    }
}
