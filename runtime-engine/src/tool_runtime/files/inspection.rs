...
impl ToolRuntime {
    ...
    async fn read_one_validated_project_file(
        ...
        self.finish_resolved_project_file_read(
            request_id,
            rx,
            path,
            start_line,
    ...
    pub(crate) fn prepare_resolved_project_file_read_request(
        &self,
        proj: &ProjectConfig,
        path: String,
        start_line: Option<usize>,
        limit: Option<usize>,
        deadline: Option<Instant>,
    ) -> Result<ShellFileOpRequest, ToolResult> {
        if deadline.is_some_and(|deadline| Instant::now() >= deadline) {
            return Err(read_file_failure(ReadFileReason::Timeout, Some(&path)));
        }
        if let Some(failure) = validate_read_file_path(&path) {
            return Err(failure);
        }
        ...
        Ok(ShellFileOpRequest {
            op: "read".to_string(),
            client_id: proj.client_id.clone(),
            path,
            cwd: Some(proj.path.clone()),
            content: None,
            // The shared scanner bounds raw content. The Runner preflights the
            // complete v1 envelope against this cap, and ToolRuntime separately
            // checks final model output after numbering/JSON escaping.
    ...
    pub(crate) async fn finish_resolved_project_file_read(
        &self,
        request_id: String,
    ...
    pub(crate) async fn load_project_instructions(
        ...
        self.load_project_instructions_for_auth(config, None).await
    }
    ...
    pub(crate) async fn load_coding_project_instructions(
        ...
        self.load_coding_project_instructions_for_auth(config, None)
            .await
    }
