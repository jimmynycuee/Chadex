...
impl ToolRuntime {
    ...
    pub(crate) async fn authorize_runner_tool(
        ...
        let capability_names = required
            .as_ref()
            .filter(|required| !required.is_owner_only())
            .map(|required| required.registry_capabilities())
            .unwrap_or_default();
        let supported = self
            .runner_registry
            .runner_supports_any_for_auth(&client_id, &capability_names, access.as_ref())
            .await
