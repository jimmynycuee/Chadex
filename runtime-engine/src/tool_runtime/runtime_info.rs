...
impl ToolRuntime {
    ...
    pub(crate) async fn runtime_status(&self, auth: Option<&AuthContext>) -> ToolResult {
        ...
        // -- tools summary ----------------------------------------------------
        // Status needs only canonical names, not every tool's full input/output
        // schema. Keep live authority/Runner observations above uncached.
        let tools_names: Vec<_> = model_visible_tool_definitions().map(|s| s.name).collect();
    ...
    async fn runtime_status_for_client(
        ...
        let tool_names: Vec<_> = model_visible_tool_definitions().map(|s| s.name).collect();
        let tools = json!({
