    ...
    for entry in entries.flatten() {
        ...
        if !state.terminal() {
            let effects = receipt
                ...
                .and_then(Value::as_bool)
                != Some(false);
            let active_job_id = receipt
                .pointer("/result/counters/active_job_id")
                .and_then(Value::as_str)
                .filter(|job_id| !job_id.trim().is_empty())
                .map(str::to_string);
            let state = if effects || active_job_id.is_some() {
                "unknown"
            } else {
                "interrupted"
            };
            ...
            receipt["result"]["failure"] = if let Some(job_id) = active_job_id {
                json!({
                    "kind":"process_lost",
                    "category":"process",
                    "reason_code":"runtime_restart_process_lost",
                    "message":"Chadex restarted while this execution still owned a nonterminal Runner Job; the original execution is preserved as unknown and will not be retried automatically",
                    "job_id":job_id,
            ...
            if receipt["result"]["workspace"].is_object() {
                ...
                if receipt["result"]["failure"]["kind"] == json!("process_lost") {
                    receipt["result"]["workspace"]["preserve_reason"] =
                        json!("runtime_restart_process_lost");
                }
            }
