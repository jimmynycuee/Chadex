...
impl ToolRuntime {
    ...
    pub(crate) async fn prepare_managed_worktree(
        ...
        self.prepare_managed_worktree_with_options(
            client_id,
            path,
            base_ref,
            operation_id,
    ...
    /// Prepare an isolated managed worktree, optionally snapshotting the
    /// caller's complete dirty tree through a temporary Runner-side index.
    /// Keeping the option here centralizes the project/Runner lifecycle instead
    /// of making task execution issue raw Git commands against the source.
    pub(crate) async fn prepare_managed_worktree_with_options(
        &self,
        client_id: String,
        path: String,
        base_ref: Option<String>,
        ...
        if include_dirty_snapshot {
            payload["include_dirty_snapshot"] = json!(true);
        }
        let first = self
            .submit_project_op(
