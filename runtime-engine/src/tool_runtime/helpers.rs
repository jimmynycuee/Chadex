...
pub(crate) fn command_failure_kind_from_stderr(stderr: &str) -> &'static str {
    let lower = stderr.to_ascii_lowercase();
    let cwd_failure = lower.contains("unable to read current working directory")
        || lower.contains("cannot read current working directory")
        || lower.contains("getcwd");
    let permission_failure = lower.contains("operation not permitted")
        || lower.contains("permission denied")
        || lower.contains("access is denied");
    if cwd_failure && permission_failure {
        "cwd_access_denied"
    } else {
...
pub(crate) fn command_failed_message(
    ...
    let guidance = if command_failure_kind_from_stderr(stderr_tail) == "cwd_access_denied" {
        "the OS denied access to the command working directory. Retry once after confirming the project folder is still accessible. On macOS, verify Chadex Files & Folders access and relaunch Chadex after rebuilding instead of replacing a running Chadex.app bundle."
    } else {
        "inspect stderr/stdout above, then fix the reported issue or use a narrower tool."
    };
    format!(
        "Command exited with status {}.\nNo files were modified by WebCodex itself; command side effects, if any, are from the invoked command.\nstdout_tail:\n{}\nstderr_tail:\n{}\nRetry guidance: {}",
        status, stdout_tail, stderr_tail, guidance
    )
}
...
mod tests {
    ...
    #[test]
    fn cwd_permission_failure_is_classified_separately() {
        ...
        assert_eq!(
            command_failure_kind_from_stderr("ordinary failure"),
            "command_exit_nonzero"
        );
    }
