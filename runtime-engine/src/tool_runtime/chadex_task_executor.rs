...
const TASK_JOB_STOP_RECONCILE_SECS: u64 = 5;
...
fn task_process_class(executable: &str, args: &[String]) -> &'static str {
    ...
    match executable.as_str() {
        "cargo" if first == Some("test") => "test",
        "cargo" if matches!(first, Some("build" | "check" | "rustc" | "package")) => "build",
        "rustc" | "rustc.exe" => "build",
        "swift" if first == Some("test") => "test",
        "swift" if matches!(first, Some("build" | "package")) => "build",
        "swiftc" | "swiftc.exe" => "build",
        "xcodebuild" if has("test") || has("test-without-building") => "test",
        "xcodebuild" => "build",
        _ => "generic",
    }
}

fn failed_step_process_class(step: &ChadexTaskStep, summary: Option<&Value>) -> &'static str {
    match step {
        ChadexTaskStep::RunProcess {
            executable, args, ..
        } => task_process_class(executable, args),
        ChadexTaskStep::Validate { checks } => summary
            .and_then(|value| value.get("failed_check"))
            .and_then(Value::as_u64)
            .and_then(|index| checks.get(index as usize))
            .map(|check| task_process_class(&check.executable, &check.args))
            .unwrap_or("generic"),
        _ => "generic",
    }
}
...
fn task_failure_reason(
    ...
    if let Some(reason) = result.output.get("failure_kind").and_then(Value::as_str) {
        if matches!(reason, "timeout_with_progress" | "timeout_stalled") {
            return match reason {
    ...
    let cancellation_text = result
        ...
        .any(|text| {
            ...
            text.contains("cancel")
                || text.contains("stopped by request")
                || text.contains("stop requested")
        });
    if cancellation_text
        || markers.iter().any(|marker| {
            marker.contains("cancel")
                || marker.contains("stopped")
                || marker.contains("stop_requested")
        })
    {
        return "cancelled";
    }
    ...
    if result
        ...
        == Some(true)
        && result.output.get("exit_code").and_then(Value::as_i64) == Some(-1)
    {
        return "process_crash";
    }
    match failed_step_process_class(step, summary) {
        "test" => "test_failure",
        "build" => "build_failure",
        _ => "command_failure",
    }
...
fn task_failure_category(
    reason: &str,
    fallback: &str,
    existing_category: Option<&str>,
) -> &'static str {
    // An outcome-unknown mutation is a stronger safety classification than the
    // generic interruption taxonomy. Preserve it so callers never interpret an
    // ambiguous write as an ordinary resumable transport interruption.
    if existing_category == Some("ambiguous_mutation") {
        return "ambiguous_mutation";
    }
    match reason {
        "timeout_with_progress" | "timeout_stalled" => "timeout",
        "build_failure" => "build",
        "test_failure" => "test",
        ...
        _ if fallback == "validate" => "validation",
        _ => "step",
    }
}

fn task_retry_strategy(reason: &str) -> &'static str {
    match reason {
        "timeout_with_progress" => "preserve_cache_resume_or_expand_window",
        "timeout_stalled" => "diagnose_before_retry",
        "interrupted" => "preserve_cache_resume_if_inputs_valid",
        "process_crash" => "diagnose_then_bounded_retry",
        "build_failure" | "test_failure" => "no_automatic_retry",
        "cancelled" => "no_retry",
        "task_deadline" => "preserve_cache_resume_or_expand_window",
        _ => "caller_decides",
    }
}
...
fn write_private_projection_file(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    if path.exists() && fs::symlink_metadata(path)?.file_type().is_symlink() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::PermissionDenied,
            "refusing task state symlink",
...
fn persist_task_state(control: &TaskControl) {
    ...
    // Sample the newest state only after acquiring the projection writer lock.
    // This prevents a delayed older update from overwriting a newer projection.
    let Ok(snapshot) = control.snapshot.lock().map(|snapshot| snapshot.clone()) else {
        return;
    };
    let Some(root) = task_storage_dir() else {
        return;
    ...
    // These files are reconstructible UI projections. Atomic rename prevents
    // torn JSON, while durable effect receipts remain on the fsync path.
    let _ = write_private_projection_file(
        &state_dir.join(format!("{}.json", snapshot.task_id)),
        &bytes,
    );
    let _ = write_private_projection_file(&state_dir.join("latest.json"), &bytes);
}
...
impl ToolRuntime {
    ...
    async fn dispatch_task_step(
        ...
        let outcome = self
            .dispatch_task_step_inner(
                control, project, session_id, step, policy, remaining, auth, transport,
            )
            .await;
    ...
    async fn await_task_process_job(
        &self,
        control: &TaskControl,
        ...
        let effective_timeout_secs = result
            .output
            .get("effective_timeout_secs")
            .and_then(Value::as_u64)
            .unwrap_or_else(|| remaining.as_secs().max(1));
        let access = crate::runner_http::runner_access_from_auth(auth);
        let deadline = Instant::now() + remaining;
        let mut stop_reason: Option<&'static str> = None;
        let mut stop_reconcile_deadline: Option<Instant> = None;
        let _ = update_snapshot(control, |snapshot| {
            snapshot.counters["active_job_id"] = json!(job_id);
            snapshot.counters["active_job_state"] = json!("observing");
        });

        loop {
            if stop_reason.is_none() {
                let reason = if control.cancel_requested.load(Ordering::SeqCst) {
                    Some("cancelled")
                ...
                if let Some(reason) = reason {
                    match self
                        ...
                        .stop_job_for_auth(
                            access.as_ref(),
                            &job_id,
                            "chadex_task_executor".to_string(),
                        )
                        .await
                    {
                        Ok(_) => {
                            stop_reason = Some(reason);
                            stop_reconcile_deadline = Some(
                                Instant::now() + Duration::from_secs(TASK_JOB_STOP_RECONCILE_SECS),
                            );
                            let _ = update_snapshot(control, |snapshot| {
                                snapshot.counters["active_job_state"] = json!("stop_requested");
                                snapshot.counters["active_job_stop_reason"] = json!(reason);
                            });
                        }
                        Err(error) => {
                            let mut unknown = super::process::outcome_unknown_result(format!(
                                "the task could not stop its existing Job during {reason}: {error}"
                            ));
                            unknown.output["job_id"] = json!(job_id);
                            unknown.output["task_stop_reason"] = json!(reason);
                            return unknown;
                        }
                    }
                }
            } else if stop_reconcile_deadline.is_some_and(|deadline| Instant::now() >= deadline) {
                let reason = stop_reason.unwrap_or("interrupted");
                let mut unknown = super::process::outcome_unknown_result(format!(
                    "the task requested Job stop for {reason}, but no trustworthy terminal result arrived within {TASK_JOB_STOP_RECONCILE_SECS}s"
                ));
                unknown.output["job_id"] = json!(job_id);
                unknown.output["task_stop_reason"] = json!(reason);
                return unknown;
            ...
            match self
                ...
                .await
            {
                Ok(job) if super::jobs::is_terminal_job_status(&job.status) => {
                    let terminal = self
                        .runner_registry
                        .hidden_job_log_for_auth(access.as_ref(), &job_id, None)
                        .await;
                    let (job, stdout, stderr, _, _) = match terminal {
                        Ok(value) => value,
                        Err(error) => {
                            return ToolResult::err_with_output(
                                format!("the task could not read terminal Job state: {error}"),
                                json!({
                                    "execution_state":"outcome_unknown",
                                    "command_started":true,
                                    "command_completed":false,
                                    "job_id":job_id,
                                    "failure_kind":"runner_disconnect",
                                }),
                            )
                        }
                    };
                    let mut terminal_result = super::process::terminal_structured_job_result(
                        &job,
                        stdout.unwrap_or_default(),
                        stderr.unwrap_or_default(),
                        effective_timeout_secs,
                    );
                    terminal_result.output["promoted_to_job"] = json!(true);
                    terminal_result.output["terminal"] = json!(true);
                    terminal_result.output["job_id"] = json!(job_id);
                    terminal_result.output["job_status"] = json!(job.status);
                    terminal_result.output["effective_timeout_secs"] =
                        json!(effective_timeout_secs);
                    if let Some(reason) = stop_reason {
                        terminal_result.output["task_stop_reason"] = json!(reason);
                    }
                    self.runner_registry
                        .remove_projected_hidden_structured_job_record(&job_id)
                        .await;
                    let _ = update_snapshot(control, |snapshot| {
                        snapshot.counters["active_job_id"] = Value::Null;
                        snapshot.counters["active_job_state"] = json!("terminal");
                    });
                    return terminal_result;
                }
                Ok(job) => {
                    result.output["job_status"] = json!(job.status);
                    result.output["execution_state"] = json!(if job.started_at.is_some() {
                        "running"
                    } else {
                        "queued"
                    });
                ...
                Err(error) => {
                    return ToolResult::err_with_output(
                        format!("the task lost observation of its existing Job: {error}"),
                        json!({
                            "execution_state":"outcome_unknown",
                            "command_started":true,
                            "command_completed":false,
        ...
        match step {
            ...
            ChadexTaskStep::RunProcess {
                ...
                let result = self
                    ...
                    .await;
                let result = self
                    .await_task_process_job(control, result, remaining, auth)
                    .await;
                let summary = compact_result_summary(kind, &result);
                (result, summary, false)
            }
    ...
    async fn execute_package_range(
        ...
        for index in start_step..=end_step {
            ...
            if !result.success {
                ...
                let reason = task_failure_reason(&step, &result, summary_for_failure.as_ref());
                let failure_status =
                    if reason == "cancelled" && control.cancel_requested.load(Ordering::SeqCst) {
                        "cancelled"
                ...
                let category = task_failure_category(
                    reason,
                    &kind,
                    failure.get("category").and_then(Value::as_str),
                );
                failure["category"] = json!(category);
                failure["retry_strategy"] = json!(task_retry_strategy(reason));
    ...
    async fn execute_parallel_package_task(
        ...
        if let Some((status, kind, message)) = terminal_failure {
            ...
            super::execution_workspace::persist_workspace_projection_best_effort(&integration);
            ...
            if error == "execution_workspace_source_overlap" {
                    ...
                    .await
                {
                    ...
                    if reconciled {
                        ...
                        super::execution_workspace::persist_workspace_projection_best_effort(
                            &reconciliation,
                        );
                        let cleanup_started = Instant::now();
                        if let Err(cleanup_error) = self
                            .cleanup_execution_workspace(&mut reconciliation, auth)
                            .await
                        {
        ...
        for (package_index, (workspace, _)) in package_results.iter().enumerate() {
            ...
            cleanup_jobs.spawn(async move {
                ...
                let error = match runtime
                    .cleanup_execution_workspace(&mut workspace, auth.as_ref())
                    .await
                {
                    Ok(()) => {
                ...
                super::execution_workspace::persist_workspace_projection_best_effort(&workspace);
        ...
        if cleanup_error.is_none() {
            let cleanup_started = Instant::now();
            match self
                .cleanup_execution_workspace(&mut integration, auth)
                .await
            {
                Ok(()) => {
        ...
        let cleanup_pending = cleanup_error.clone();
        if let Some(error) = cleanup_error {
            integration.state = WorkspaceState::Preserved;
            integration.preserve_reason = Some(error);
            super::execution_workspace::persist_workspace_projection_best_effort(&integration);
        }
        ...
        let _ = update_snapshot(&control, |snapshot| {
            ...
            if let Some(error) = cleanup_pending.as_deref() {
                snapshot.counters["cleanup_pending"] = json!(true);
                snapshot.counters["cleanup_error"] = json!(bounded_message(error));
                snapshot.workspace["cleanup_pending"] = json!(true);
            }
        });
    ...
    pub(crate) async fn execute_chadex_task(
        ...
        // The request is only an observer of the admitted task. Keep the actual
        // supervisor owned by the runtime so an HTTP/MCP disconnect or outer
        // observation timeout cannot drop the execution future and turn a live
        // Runner Job into a false terminal interruption. The execution receipt,
        // TaskControl and exact Job id remain the single lifecycle authority.
        let runtime = self.clone();
        let auth = auth.cloned();
        ...
        .await
        {
            ...
            Err(error) => ToolResult::err_with_output(
                format!("task supervisor join failed: {error}"),
                json!({
                    "error_kind":"task_supervisor_join_failed",
                    "execution_state":"outcome_unknown",
                    "state_changed":true,
                }),
            ),
    ...
    async fn execute_chadex_task_attempt(
            ...
            .await
        {
            ...
            Err(error) => {
                let _ = update_snapshot(&control, |snapshot| {
                    snapshot.execution.effects_possible =
                        execution::uncertain_workspace_error(&error);
                    snapshot.status = "blocked".to_string();
                    snapshot.finished_at_ms = Some(now_ms());
                    snapshot.duration_ms = Some(elapsed_ms(started));
                    snapshot.failure = Some(json!({
        ...
        for (index, step) in steps.into_iter().enumerate() {
            ...
            if !result.success {
                ...
                let reason = task_failure_reason(&step, &result, summary.as_ref());
                let failure_status =
                    if reason == "cancelled" && control.cancel_requested.load(Ordering::SeqCst) {
                        "cancelled"
                    } else if kind == "validate" {
        ...
        // Cleanup happens after the execution effect is already known. Keep
        // teardown debt separate from the task outcome so a successful apply
        // cannot later be rewritten as failed or unknown solely because a
        // managed worktree still needs recovery cleanup.
        let mut cleanup_pending: Option<String> = None;
        ...
        if let Some((_, _, message)) = terminal_failure {
                            ...
                            .await
                        {
                            ...
                            if checks_ok {
                                    ...
                                    .await
                                {
                                    ...
                                    cleanup_pending = Some(cleanup_error.clone());
                                    let _ = update_snapshot(&control, |snapshot| {
                                        snapshot.workspace = projection;
                                        snapshot.counters["cleanup_pending"] = json!(true);
                                        snapshot.counters["cleanup_error"] =
                                            json!(bounded_message(&cleanup_error));
                                    });
                                }
                                reconciliation.timings.cleanup_ms =
                                    cleanup_started.elapsed().as_millis() as u64;
            ...
            let cleanup_started = Instant::now();
            if let Err(error) = self.cleanup_execution_workspace(&mut workspace, auth).await {
                workspace.state = WorkspaceState::Preserved;
                workspace.preserve_reason = Some(error.clone());
                super::execution_workspace::persist_workspace_projection_best_effort(&workspace);
                let projection = workspace.projection();
                cleanup_pending = Some(match cleanup_pending.take() {
        ...
        match action.as_str() {
            "list" => {
                ...
                let manifests = super::execution_workspace::load_workspace_manifests()
                    ...
                    .collect::<Vec<_>>();
                let prepare_intents = super::execution_workspace::load_workspace_prepare_intents()
                    .into_iter()
                    .filter(|intent| intent.source_project == project)
                    .collect::<Vec<_>>();
                ...
                return ToolResult::ok(json!({
                    ...
                    "workspace_prepare_intent_count": prepare_intents.len(),
            ...
            "discard" => {
                ...
                let mut workspaces = match workspaces {
                ...
                let prepare_intents = super::execution_workspace::load_workspace_prepare_intents()
                    .into_iter()
                    .filter(|intent| intent.task_id == task_id && intent.source_project == project)
                    .collect::<Vec<_>>();
                for intent in prepare_intents {
                    if workspaces.iter().any(|workspace| {
                        workspace.prepare_operation_id.as_deref()
                            == Some(intent.operation_id.as_str())
                    }) {
                        if let Err(error) =
                            super::execution_workspace::remove_workspace_prepare_intent(&intent)
                        {
                            cleanup_errors.push(format!(
                                "execution_workspace_prepare_intent_remove_failed:{error}"
                            ));
                        }
                        continue;
                    }
                    match self.reconcile_workspace_prepare_intent(&intent, auth).await {
                        Ok(workspace) => workspaces.push(workspace),
                        Err(error) => cleanup_errors.push(format!(
                            "execution_workspace_prepare_reconcile_failed:{error}"
                        )),
                    }
                }
...
mod tests {
    ...
    #[test]
    fn phase13_failure_reasons_and_retry_strategies_are_reason_aware() {
        ...
        let timed_progress = ToolResult::err_with_output(
            "timed out",
            json!({
                "failure_kind": "timeout_with_progress",
        ...
        assert_eq!(
            task_failure_reason(&build, &timed_progress, None),
            "timeout_with_progress"
        );
        assert_eq!(
            task_retry_strategy("timeout_with_progress"),
            "preserve_cache_resume_or_expand_window"
        );

        let timed_stalled = ToolResult::err_with_output(
            "stalled",
            json!({
                "failure_kind": "timeout_stalled",
                "execution_state": "timed_out",
                "command_completed": false,
                "exit_code": -1
            }),
        ...
        assert_eq!(
            task_failure_reason(&build, &timed_stalled, None),
            "timeout_stalled"
        );
        assert_eq!(
            task_retry_strategy("timeout_stalled"),
            "diagnose_before_retry"
        );

        let build_failure = ToolResult::err_with_output(
            "compiler failed",
            json!({
        ...
        assert_eq!(
            task_failure_reason(&build, &build_failure, None),
            "build_failure"
        );
        assert_eq!(task_retry_strategy("build_failure"), "no_automatic_retry");

        let tests = ChadexTaskStep::Validate {
            checks: vec![ChadexTaskProcess {
                executable: "cargo".to_string(),
        ...
        assert_eq!(
            task_failure_reason(&tests, &build_failure, Some(&json!({"failed_check": 0}))),
            "test_failure"
        );
        assert_eq!(task_retry_strategy("test_failure"), "no_automatic_retry");

        let crash = ToolResult::err_with_output(
            "process crashed",
            json!({
                "failure_kind": "command_exit_nonzero",
                "execution_state": "completed",
        ...
        assert_eq!(task_failure_reason(&build, &crash, None), "process_crash");

        let cancelled = ToolResult::err_with_output(
            "command failed",
            json!({
                "failure_kind": "command_exit_nonzero",
                "execution_state": "completed",
                "command_completed": true,
                "exit_code": -1,
                "stderr_tail": "job stopped by request"
            }),
        );
        assert_eq!(task_failure_reason(&build, &cancelled, None), "cancelled");

        let interrupted = ToolResult::err_with_output(
            "unknown outcome",
            json!({"failure_kind": "outcome_unknown", "execution_state": "outcome_unknown"}),
        );
        assert_eq!(
            task_failure_reason(&build, &interrupted, None),
            "interrupted"
        );
    }
    ...
    #[test]
    fn startup_recovery_preserves_same_execution_and_marks_active_job_process_lost() {
        let dir = tempfile::tempdir().unwrap();
        let state_dir = dir.path().join("state");
        fs::create_dir_all(&state_dir).unwrap();
        let task_id = "chadex_task_11111111111111111111111111111111";
        let job_id = "wc_job_restart_fixture";
        let projection = json!({
            "task_id": task_id,
            "execution_id": task_id,
            "execution_state": "running",
            "project": "project-process-lost",
            "source_path": "/tmp/project-process-lost",
            "goal": "keep the same execution identity after restart",
            "status": "running",
            "started_at_ms": 4321,
            "counters": {
                "active_job_id": job_id,
                "active_job_state": "observing"
            },
            "workspace": {
                "mode": "task",
                "state": "active",
                "source_project": "project-process-lost",
                "execution_project": "execution-process-lost",
                "worktree_created": true
            }
        });
        write_private_file(
        ...
        let mut execution = Execution::claim(dir.path(), task_id, None).unwrap();
        execution.state.transition(ExecutionState::Running).unwrap();
        execution.effects_possible = true;
        execution.persist(projection).unwrap();
        drop(execution);

        let recovered = load_recovered_task_projections_at(dir.path());
        let value = recovered.get(task_id).expect("same execution recovered");
        assert_eq!(value["task_id"], task_id);
        assert_eq!(value["execution_id"], task_id);
        assert_eq!(value["status"], "unknown");
        assert_eq!(value["execution_state"], "unknown");
        assert_eq!(value["failure"]["kind"], "process_lost");
        assert_eq!(value["failure"]["category"], "process");
        assert_eq!(value["failure"]["job_id"], job_id);
        assert_eq!(
            value["failure"]["reason_code"],
            "runtime_restart_process_lost"
        );
        assert_eq!(value["failure"]["retry_strategy"], "no_automatic_retry");
        assert_eq!(value["workspace"]["state"], "preserved");
        assert_eq!(
            value["workspace"]["preserve_reason"],
            "runtime_restart_process_lost"
        );
        assert_eq!(value["recovery"]["actions"], json!(["discard"]));
        ...
        .unwrap();
        assert_eq!(receipt["execution"]["execution_id"], task_id);
        assert_eq!(receipt["execution"]["state"], "unknown");
        assert_eq!(receipt["result"]["failure"]["kind"], "process_lost");
    }
