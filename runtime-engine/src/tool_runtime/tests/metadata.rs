...
async fn runtime_status_includes_build_metadata() {
    ...
    let expected_names: Vec<_> = registered_tool_specs()
        .into_iter()
        .map(|spec| spec.name)
        .collect();
    assert_eq!(result.output["tools"]["names"], json!(expected_names));
    assert_eq!(result.output["tools"]["count"], expected_names.len());
    assert_eq!(
