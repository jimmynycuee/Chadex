...
#[tokio::test]
async fn phase15_coding_start_and_resume_emit_content_free_subphases() {
    ...
    env.set(
        ...
        trace_root.path().to_str().unwrap(),
    );
    let root = tempfile::tempdir().unwrap();
    seed_coding_repository(root.path(), "private rule body");
    let runtime = ToolRuntime::new_for_tests();
    let project = register_runner_project_at_path(&runtime, "phase15", "demo", root.path()).await;
    let auth = auth_context(None, true);

    let first_trace = crate::tool_request_trace::new_trace_id();
    let first = crate::tool_request_trace::scope_active_trace(
        Some(first_trace.clone()),
        dispatch_coding_call_in_window(
            &runtime,
            "phase15",
            work_on_project_call(&project, "start", None),
            Some(&auth),
            "phase15-window",
        ),
    )
    ...
    let resumed_trace = crate::tool_request_trace::new_trace_id();
    let resumed = crate::tool_request_trace::scope_active_trace(
        Some(resumed_trace.clone()),
        dispatch_coding_call_in_window(
            &runtime,
            "phase15",
            work_on_project_call(&project, "continue", Some(session_id)),
            Some(&auth),
            "phase15-window",
        ),
    )
    .await;
    assert!(resumed.success, "{:?}", resumed.error);
    let handoff_trace = crate::tool_request_trace::new_trace_id();
    let handoff = crate::tool_request_trace::scope_active_trace(
        Some(handoff_trace.clone()),
        runtime.session_handoff_summary(
    ...
    let fresh_events =
        fs::read_to_string(trace_root.path().join(&first_trace).join("events.jsonl")).unwrap();
    let resumed_events =
        fs::read_to_string(trace_root.path().join(&resumed_trace).join("events.jsonl")).unwrap();
    let handoff_events =
        fs::read_to_string(trace_root.path().join(&handoff_trace).join("events.jsonl")).unwrap();
    assert!(fresh_events.contains("coding_context_probes"));
    assert!(fresh_events.contains("coding_session_create_ensure"));
    assert!(fresh_events.contains("work_on_project_prepared"));
    assert!(resumed_events.contains("coding_session_resume_lookup"));
    assert!(resumed_events.contains("coding_session_resume_ensure"));
    assert!(resumed_events.contains("work_on_project_resume_requested_prepared"));
    assert!(handoff_events.contains("session_handoff_built"));
    assert!(!fresh_events.contains("private rule body"));
    assert!(!resumed_events.contains("private rule body"));
    assert!(!handoff_events.contains("private rule body"));
    if let Ok(output) = std::env::var("CHADEX_PHASE15_DOGFOOD_DIR") {
        for trace_id in [&first_trace, &resumed_trace, &handoff_trace] {
            let destination = Path::new(&output).join(trace_id);
            fs::create_dir_all(&destination).unwrap();
            fs::copy(
                trace_root.path().join(trace_id).join("events.jsonl"),
