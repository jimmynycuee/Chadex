...
use crate::runner_protocol::{
    RunnerCapabilities, RunnerPollRequest, RunnerRequest, RunnerResultRequest,
    RunnerTextSearchPatternMode, RunnerTextSearchRequest, RunnerTextSearchResultMode,
    EXTERNAL_SEARCH_REQUEST_PREFIX, SEARCH_TEXT_REQUEST_KIND,
};
...
#[tokio::test]
async fn search_project_texts_prefers_typed_runner_search_when_capability_is_advertised() {
    let root = tempfile::tempdir().unwrap();
    let runtime = ToolRuntime::new_for_tests();
    let client_id = "typed-search-runner";
    ...
    capabilities.structured_search_text = true;
    ...
    let mut typed_query = query("needle.literal", Some(SearchResultMode::Matches));
    typed_query.pattern_mode = Some(SearchPatternMode::Literal);
    typed_query.path = Some("src".to_string());
    typed_query.include_globs = Some(vec!["*.rs".to_string()]);
    let task = tokio::spawn({
        let runtime = runtime.clone();
        async move {
            runtime
                .search_project_texts("demo".to_string(), vec![typed_query])
                .await
        }
    });

    let request = wait_for_patch_agent_request(&runtime, client_id).await;
    assert_eq!(request.kind, SEARCH_TEXT_REQUEST_KIND);
    assert!(request.command.is_empty());
    assert!(request.process.is_none());
    assert!(request.script.is_none());
    let payload: RunnerTextSearchRequest =
        serde_json::from_str(request.stdin.as_deref().expect("typed search payload")).unwrap();
    assert_eq!(payload.pattern, "needle.literal");
    assert_eq!(payload.pattern_mode, RunnerTextSearchPatternMode::Literal);
    assert_eq!(payload.path, "src");
    assert_eq!(payload.include_globs, vec!["*.rs"]);
    assert_eq!(payload.result_mode, RunnerTextSearchResultMode::Matches);

    complete_search_success(&runtime, client_id, &request, "src/lib.rs").await;
    let result = task.await.unwrap();
...
#[tokio::test]
async fn search_project_texts_keeps_posix_fallback_for_runner_without_typed_search_capability() {
    let root = tempfile::tempdir().unwrap();
    let runtime = ToolRuntime::new_for_tests();
    let client_id = "legacy-search-runner";
    register_runner_project_at_path(&runtime, client_id, "demo", root.path()).await;
        ...
        async move {
            runtime
                .search_project_texts(
                    "demo".to_string(),
                    vec![query("needle", Some(SearchResultMode::Matches))],
                )
                .await
        }
    });
    ...
    assert!(request.command.starts_with(EXTERNAL_SEARCH_REQUEST_PREFIX));
    complete_search_success(&runtime, client_id, &request, "src/lib.rs").await;
    let result = task.await.unwrap();
    assert!(result.success, "{:?}", result.error);
    assert_no_agent_request(&runtime, client_id).await;
