...
async fn dispatch_coding_call_in_window_with_transport(
    ...
    let active_trace_id = crate::tool_request_trace::current_active_trace_id();
    let task = tokio::spawn({
        let runtime = runtime.clone();
        let auth = auth.cloned();
        let window_id = window_id.to_string();
        async move {
            let _trace_env_reader = active_trace_id
                .as_ref()
                .map(|_| crate::test_support::TestToolRequestTraceEnvReaderGuard::new());
            let window = ClientWindow::for_test(&window_id);
            crate::tool_request_trace::scope_active_trace(
                active_trace_id,
                runtime.dispatch_with_auth_transport_options_and_metadata_with_window(
                    call,
                    auth.as_ref(),
                    transport,
                    Default::default(),
