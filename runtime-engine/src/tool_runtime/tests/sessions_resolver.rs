...
#[tokio::test]
async fn exact_project_resolution_cache_invalidates_on_project_change() {
    let runtime = runtime_with_resolver_projects().await;
    let project = "agent:workstation:my-repo";

    let first = runtime.resolve_project_input(project).await.unwrap();
    assert_eq!(first.config.path, "/root/git/workstation-my-repo");

    let runner = runtime
        .runner_registry
        .get_runner_view("workstation")
        .await
        .expect("workstation runner");
    runtime
        .runner_registry
        .upsert_runner_project_for_instance(
            ...
            named_registered_project(
                ...
                "my-repo",
                "My Repo",
                "/root/git/workstation-my-repo-v2",
                300,
            ),
        )
        .await
    ...
    let second = runtime.resolve_project_input(project).await.unwrap();
    assert_eq!(second.config.path, "/root/git/workstation-my-repo-v2");
}
