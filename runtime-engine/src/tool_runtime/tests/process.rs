...
#[tokio::test]
async fn adaptive_direct_process_waits_through_runner_hard_deadline() {
    let temp = tempfile::tempdir().unwrap();
    let runtime = test_runtime();
    let project =
        register_process_agent(&runtime, "process-adaptive-direct", temp.path(), true).await;
    let task = tokio::spawn({
        let runtime = runtime.clone();
        async move {
            runtime
    ...
    let request = wait_for_patch_agent_request(&runtime, "process-adaptive-direct").await;
    assert_eq!(request.kind, "run_process");
    assert_eq!(request.timeout_secs, 1);
    // A Rust build has a 4x bounded adaptive hard deadline. Returning after the
    // old soft+2 server wait proves the server now observes the same contract
    // instead of reporting outcome_unknown while the Runner is still healthy.
    tokio::time::sleep(Duration::from_millis(3_200)).await;
    complete_process_lifecycle(
        &runtime,
        "process-adaptive-direct",
        request.request_id,
        ShellCommandExecutionState::Completed,
        Some(0),
        "built",
...
#[tokio::test]
async fn adaptive_build_promotes_when_hard_deadline_outlives_soft_sync_window() {
    let temp = tempfile::tempdir().unwrap();
    let runtime = test_runtime().with_structured_execution_sync_wait(Duration::from_millis(40));
    let project = register_process_job_agent(&runtime, "process-adaptive-build", temp.path()).await;
    let auth = auth_context(None, true);
    let (task, request) = dispatch_process_until_request(
        &runtime,
        "process-adaptive-build",
        ToolCall::RunProcess {
            project,
    ...
    assert_eq!(
        ...
        "Runner still owns the 30s soft timeout"
    );
    update_process_job(
        &runtime,
        "process-adaptive-build",
    ...
    assert_eq!(handoff.output["terminal"], false);
    assert_eq!(handoff.output["execution_state"], "running");
    assert_eq!(handoff.output["effective_timeout_secs"], 30);
    assert_eq!(handoff.output["sync_wait_secs"], 30);
    ...
    .await;
    let terminal = runtime
        .job_status_for_auth(job_id.to_string(), false, Some(&auth))
        .await;
    assert!(terminal.success, "{:?}", terminal.error);
    assert_eq!(terminal.output["status"], "completed");
}
