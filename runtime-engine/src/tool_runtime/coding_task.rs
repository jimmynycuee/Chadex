...
impl ToolRuntime {
    ...
    async fn start_coding_workflow(
        ...
        let resume_session_project = match resume_session_id.as_deref() {
            Some(session_id) => {
                ...
                let lookup = self
                    ...
                    .await;
                crate::tool_request_trace::record_active_phase(
                    "coding_session_resume_lookup",
        ...
        let context_probes_started = std::time::Instant::now();
            ...
            if startup.include_repository_overview {
                let (semantic_navigation, project_instructions, repository_overview, extensions) =
                    futures_util::future::join4(
                        ...
                        self.load_coding_project_instructions_for_auth(&resolved.config, auth),
        ...
        if crate::tool_request_trace::tool_request_trace_enabled() {
            crate::tool_request_trace::record_active_phase(
                "coding_context_probes",
                context_probes_started.elapsed(),
                Some(
                    project_instructions
                        .files
                        .iter()
                        .fold(0usize, |total, file| {
                            total.saturating_add(file.content.len())
                        }),
                ),
        ...
        let session_ensure_started = std::time::Instant::now();
        let session_ensure = self.sessions.ensure_coding_session_with_git_baseline(
        ...
        crate::tool_request_trace::record_active_phase(
            if resume_requested {
                "coding_session_resume_ensure"
            } else {
                "coding_session_create_ensure"
            },
            session_ensure_started.elapsed(),
            None,
            None,
        );
        let session_outcome = match session_ensure {
            Ok(outcome) => outcome,
    ...
    pub(crate) async fn work_on_project(
        ...
        crate::tool_request_trace::record_active_phase(
            if resume_requested {
                "work_on_project_resume_requested_prepared"
            } else {
                "work_on_project_prepared"
            },
        ...
        let projected = project_work_on_project_output_with_workflow_inner(
            projected_project,
            result.output,
...
fn project_work_on_project_output_with_workflow_inner(
    ...
    if projection.instructions.sources.len() > 16 {
        return work_on_project_projection_failed(
            "instructions.sources",
            "at most 16 source objects",
