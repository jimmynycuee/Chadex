...
pub(crate) enum ReadModelProjection {
    ...
    Batch {
        ...
        include_read_revision: Option<bool>,
    ...
    pub(crate) fn capture(call: &ToolCall) -> Self {
        match call {
            ToolCall::ReadFiles {
                ...
                include_read_revision: *include_read_revision,
...
fn read_files_suggested_arguments(
    ...
    if let Some(include_read_revision) = include_read_revision {
        arguments["include_read_revision"] = json!(include_read_revision);
    }
...
impl ToolRuntime {
    ...
    pub(crate) async fn read_files_resolved(
        ...
        self.read_files_resolved_with_revision_policy(resolved, items, with_line_numbers, true)
            .await
    }

    pub(crate) async fn read_files_resolved_with_revision_policy(
        &self,
        resolved: &ResolvedProject,
        items: Vec<ReadFilesItem>,
        with_line_numbers: Option<bool>,
        include_read_revision: bool,
    ) -> ToolResult {
        ...
        // Validate each item before admission, then enqueue all valid reads
        // under one registry observation. Runner responses remain independent
        // and are awaited concurrently below; final model output is restored
        // to deterministic source order.
        ...
        for (index, item) in items.into_iter().enumerate() {
            ...
            let requested_revision = item.expected_read_revision;
            let expected_sha256 = match requested_revision {
            ...
            let request = match self.prepare_resolved_project_file_read_request(
                &resolved.config,
                path.clone(),
                item.start_line,
                item.limit,
            ...
            prepared.push(PreparedBatchRead {
                index,
                path,
        ...
        if !requests.is_empty() {
            ...
            {
                Ok(handles) => {
                    let finished = stream::iter(prepared.into_iter().zip(handles).map(
                        |(prepared, (request_id, rx))| async move {
                            let mut result = self
                                .finish_resolved_project_file_read(
                                    request_id,
                                    rx,
                                    prepared.path.clone(),
                                    prepared.start_line,
                            ...
                            if result.success {
                                if let Some(expected_sha256) = prepared.expected_sha256.as_deref() {
                                    let actual_sha256 =
                                        result.output.get("sha256").and_then(Value::as_str);
                                    if actual_sha256 != Some(expected_sha256) {
                                        result = stale_read_revision_failure(&prepared.path);
                                    }
                                }
                            }
                            ...
                            if success {
                                let continuation_needs_revision =
                                    output.get("has_more").and_then(Value::as_bool) == Some(true);
                                let should_emit_revision = include_read_revision
                                    || continuation_needs_revision
                                    || prepared.requested_revision.is_some();
                                if let Some(sha256) = output
                                    .get("sha256")
                                    .and_then(Value::as_str)
                                    .map(str::to_string)
                                    .filter(|_| should_emit_revision)
                                {
                                    let read_revision =
                                        prepared.requested_revision.unwrap_or_else(|| {
                                            self.read_revisions.observe(prepared.target, sha256)
                                        });
                ...
                Err(_) => {
                    completed.extend(prepared.into_iter().map(|prepared| {
                        ...
                        json!({
                            ...
                            "error": "read_file failed: agent_unavailable",
                        })
                    }));
                }
            }
        ...
        // Server-resolved canonical identity evidence for the native
        // verification boundary. The verifier must never trust a path supplied
        // only by the incoming ChatGPT request.
        output["project_path"] = json!(resolved.config.path.clone());
...
mod tests {
    ...
    #[test]
    fn project_identity_proof_survives_model_projection() {
        let mut output = batch_output(
            "agent:oe:demo",
            1,
            vec![default_complete_item(0, &["identity".to_string()])],
            false,
            None,
            None,
        );
        output["project_path"] = json!("/tmp/project");
        let projection = batch_projection(1, None);
        let mut result = ToolResult::ok(output);

        apply_model_facing_output_budget(&mut result, None, &projection);
        super::super::dispatch::sparsify_complete_read_success("read_files", &mut result);

        assert_eq!(result.output["project_path"], "/tmp/project");
