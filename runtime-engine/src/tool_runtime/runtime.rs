...
use super::project_resolution::ProjectResolutionCache;
...
pub struct ToolRuntime {
    ...
    pub(crate) chadex_tasks: Arc<super::chadex_task_executor::ChadexTaskStore>,
    ...
    /// Small process-local cache for exact full runtime Project ids. Entries
    /// are partitioned by RunnerAccess and fenced by RunnerRegistry's routing
    /// epoch, so authority or inventory changes invalidate them immediately.
    pub(crate) project_resolution_cache: Arc<StdMutex<ProjectResolutionCache>>,
...
impl ToolRuntime {
    pub fn new(runner_registry: Arc<RunnerRegistry>, runtime_info: Arc<RuntimeInfo>) -> Self {
        Self {
            ...
            chadex_tasks: Arc::new(super::chadex_task_executor::ChadexTaskStore::default()),
            runtime_info,
            #[cfg(feature = "workspace-checkpoints")]
            ...
            project_resolution_cache: Arc::new(StdMutex::new(ProjectResolutionCache::default())),
            #[cfg(feature = "experimental-code-mode")]
