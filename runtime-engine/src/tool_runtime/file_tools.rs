    ...
    pub(crate) async fn dispatch_file_tool(
        ...
        match call {
            ...
            ToolCall::ReadFiles {
                ...
                Some(Ok(resolved)) => {
                    self.read_files_resolved_with_revision_policy(
                        &resolved,
                        items,
                        with_line_numbers,
                        include_read_revision.unwrap_or(false),
                    )
                    .await
                }
            ...
            ToolCall::WriteProjectFile {
                ...
                Some(Ok(resolved)) => {
                    self.write_project_file_resolved(
                        &resolved,
                        path,
                        content,
                        overwrite,
            ...
            ToolCall::ApplyTextEdits {
                ...
                Some(Ok(resolved)) => {
                    self.apply_text_edits_resolved(&resolved, changes, dry_run)
                        .await
                }
                Some(Err(error)) => error.into_tool_result(),
                None => self.apply_text_edits(project, changes, dry_run).await,
            },
