...
use crate::runner_protocol::{ShellProcessArgv, ShellRunRequest};
...
"; fi;
    ...
    let script = r#"{ LC_ALL=C; export LC_ALL;
         { git status --porcelain=v1 -b; printf 'status_exit=%s\n' "$?"; } | {
           ...
           head_identity=$(git log -1 --no-show-signature --no-decorate --no-notes --format='%H %h' 2>/dev/null); head_exit_raw=$?;
           if [ "$head_exit_raw" -eq 0 ] 2>/dev/null; then
             head_commit=${head_identity%% *}; head_short=${head_identity#* };
           ...
           nb_base=HEAD;
           if [ -n "$head_commit" ]; then nb_base=$head_commit;
           elif ! git rev-parse --verify HEAD >/dev/null 2>&1; then nb_base=$(printf '' | git hash-object -t tree --stdin 2>/dev/null); fi;
...
impl ToolRuntime {
    ...
    pub(crate) async fn git_status(&self, project: String) -> ToolResult {
        ...
        // git_status has no shell grammar. Prefer structured argv so the hot
        // path avoids shell parsing while retaining the same Runner cwd,
        // profile environment, supervision, timeout, and authority. Older
        // compatible Runners that do not advertise structured argv keep the
        // established raw-shell fallback rather than losing functionality.
        let structured = self
            .runner_registry
            .enqueue_process(
                client_id.clone(),
            ...
            .await;
        let (req_id, rx) = match structured {
            Ok(r) => r,
            Err(error) if error.contains("capability_unavailable") => match self
                .runner_registry
                .enqueue_run(
                    ShellRunRequest {
                        client_id,
