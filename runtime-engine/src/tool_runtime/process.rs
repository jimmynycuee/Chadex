...
use super::helpers::{
    bounded_tail, command_failed_message, command_failure_kind_from_stderr,
    command_outcome_unknown_message, command_rejected_message, command_timeout_message,
    project_relative_runner_cwd, resolve_runner_cwd, COMMAND_STDIO_TAIL_CHARS,
};
use super::shell::{
...
use crate::runner_protocol::{
    classify_structured_process_command, validate_process_argv, ShellCommandExecutionState,
    ShellJobInfo, ShellJobOpRequest, ShellProcessArgv, PROCESS_CWD_MAX_BYTES,
...
pub(crate) fn command_failure_result(
    ...
    let failure_kind = if timed_out && stderr.contains("timeout_with_progress") {
        "timeout_with_progress"
    } else if timed_out && stderr.contains("timeout_stalled") {
        "timeout_stalled"
    } else if timed_out {
        "timeout"
    } else {
        command_failure_kind_from_stderr(&stderr)
    };
    let output = json!({
        "exit_code": exit_code,
        "duration_ms": duration_ms,
...
impl ToolRuntime {
    ...
    async fn run_process_with_contract_mode(
        ...
        let execution_class =
            classify_structured_process_command(&process.executable, &process.args);
        let observation_timeout_secs =
            execution_class.hard_timeout_secs(timeout, STRUCTURED_EXECUTION_TIMEOUT_MAX_SECS);
        if !async_handoff_available
            && observation_timeout_secs > STRUCTURED_EXECUTION_DIRECT_SYNC_TIMEOUT_MAX_SECS
        {
            let mut result = process_tool_failure_result(
                command_rejected_message(
                    "internal synchronous structured execution cannot observe the Runner through its bounded adaptive timeout",
                    "keep this internal operation within the direct observation budget or enable durable Job handoff.",
                ),
                "capability_unavailable",
                ShellCommandExecutionState::NotStarted,
            );
        ...
        if async_handoff_available && observation_timeout_secs > budget.sync_wait_secs {
            let job = self
                .runner_registry
                .start_job_with_metadata_for_access(
                    ShellJobOpRequest {
        ...
        // For a direct fast-path request, wait through the same bounded
        // adaptive deadline the Runner is allowed to use. Adaptive executions
        // whose hard deadline exceeds this synchronous window were promoted to
        // a durable Job above, so this remains <= the direct wait ceiling.
        let wait_timeout = observation_timeout_secs;
