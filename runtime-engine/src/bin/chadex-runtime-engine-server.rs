use chadex_runtime_engine::{server_binary_action, ServerBinaryAction};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    match server_binary_action(std::env::args().skip(1)) {
        ServerBinaryAction::Run { stop_on_stdin_eof } => {
            chadex_runtime_engine::prepare_server_process_environment()
                .map_err(std::io::Error::other)?;
            build_server_runtime()?.block_on(
                chadex_runtime_engine::run_server_with_parent_liveness(stop_on_stdin_eof),
            )
        }
