...
pub async fn tools_call(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    guard.received();
    // Authentication middleware precedes this handler; this marker is the
    // first observable API ingress boundary, not auth-start.
    guard.phase("api_handler_ingress", None);
    ...
    guard.phase("api_request_decoded", estimate_json_bytes(&body));
    ...
    .await;
    for link in &outcome.correlation.workflow_sessions {
    ...
    match outcome.error_status {
        ...
        None => {
            ...
            if guard.full_enabled() {
                if let Ok(value) = serde_json::to_value(&response) {
                    guard.capture_payload("final_response", &value);
                }
            }
            let estimated = estimate_json_bytes(&response);
